...
/* Opaque. Owns the display and the context, and whatever a backend needs to tie them
 * together. */
...
typedef struct oops_gfx_desc {
    ...
    bool
        depth; /* a depth buffer is wanted (oops-gl always has one; a hint elsewhere) */
    bool vsync; /* present paces itself to scanout (oops-gl always does; a hint
                   elsewhere) */
} oops_gfx_desc_t;

/*
 * Bring the renderer up at the requested size and make its context current on this
 * thread. Opens the display, so a title must not have opened one itself. `desc` may be
 * NULL for all defaults. Returns NULL and logs the step it stopped on; a NULL is a
 * failure to start, never a silent fall back to something slower.
 */
oops_gfx_t *oops_gfx_create(const oops_gfx_desc_t *desc);

/*
 * Present what has been drawn. Returns false only when a backend can tell the frame did
 * not reach the screen - the oops-gl backend flips on vsync and cannot report a failed
 * flip, so it returns true; the Mesa backend returns false if the flip was refused.
 */
bool oops_gfx_present(oops_gfx_t *gfx);

/* The drawable's extent - what was opened, which may differ from a size the title asked
 * for. */
void oops_gfx_extent(const oops_gfx_t *gfx, uint32_t *width, uint32_t *height);

/* The display this opened, for the callers that still need it directly - the input
 * pump, a CPU overlay surface. It belongs to the gfx and is closed by
 * `oops_gfx_destroy`; do not close it. */
