 ...
 *
 * dir_name: Directory name of the save slot (e.g. "SAVE0000" or
 * title-specific). mode: Read-only, Read-write, or Create if non-existent.
 * out_mount_path: Buffer receiving the mounted filesystem path (e.g.
 * "/savedata0"). max_path_len: Capacity of out_mount_path buffer.
 *
 * Returns: 0 on success, or negative error code.
 */
