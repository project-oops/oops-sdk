...
/*
 * **The pieces a parser and a depth sort are built out of** (2026-09-20), here rather than in
 * `src/system/libc.c` because that file is target-only and promises to be nothing but a rename -
 * so an algorithm living there could not be tested at all. `<libc/string.h>`'s `strspn`,
 * `strtok_r` and their kin, and `<libc/stdlib.h>`'s `qsort` and `bsearch`, are one-line wrappers
 * over these.
 */
...
/* `qsort`'s contract, with `qsort`'s guarantees: not stable, and the ordering of equal elements
 * is whatever the partition left. The stack is bounded at log2(count) frames - see the comment
 * on the definition for why that is not an optimisation. */
...
/* `<libc/stdio.h>`'s `sscanf` and `vsscanf` - `src/system/scanf.c`, which says why an asset
 * loader needs them and what the conversion does not promise. */
