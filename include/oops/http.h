...
typedef struct oops_http_response {
  ...
  char *body;               /* Null-terminated response payload (caller frees via oops_http_response_free) */
 ...
 * Allocates `out_resp->body` on success; caller must release via `oops_http_response_free()`.
 *
...
/**
 * Download progress callback: invoked as the body streams to disk, with the bytes written so far
 * and the total expected (`total` is 0 when the server sends no Content-Length). Called on the
 * same thread as the download, so it may draw a frame; keep it quick. `userdata` is passed
 * through unchanged.
 */
typedef void (*oops_http_progress_fn)(uint64_t downloaded, uint64_t total, void *userdata);

/**
 * Like `oops_http_get_to_file`, but reports progress: `on_progress` (may be NULL) is called after
 * each chunk is written, and once at the end. Lets a caller draw a live progress bar during an
 * otherwise-blocking download without a worker thread.
 *
 * Returns OOPS_HTTP_OK (0) on success, or a negative OOPS_HTTP_ERR_* code.
 */
