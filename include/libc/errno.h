 ...
 *
 * **The whole table, not the part someone needed.** This began as the thirty-odd names the first
 * three consumers happened to touch, and stopped at `EAGAIN` - which is where the numbering only
 * starts getting interesting, because everything above 35 is the socket and network set. libc++'s
 * `<system_error>` maps forty-two of those onto `std::errc` and names them unconditionally, so
 * Extreme Tux Racer could not compile a single translation unit that reached it: twenty errors
 * and `-ferror-limit` giving up, on `ECONNREFUSED`, `ECONNRESET`, `EDESTADDRREQ` and the rest.
 *
 * A partial table is how this file has failed twice now - once as three private shims, once here
 * - and completing it costs nothing, because these are not choices to be made one at a time.
 * They are transcribed from the FreeBSD `sys/errno.h` that oops-mesa stages as its target sysroot
 * (`oops-mesa/toolchain/sysroot/usr/include/sys/errno.h`), which is the platform's own header
 * rather than a recollection of it, and generated from it rather than typed.
 *
 * **What is deliberately absent.** FreeBSD does not define the four STREAMS errors - `ENODATA`,
 * `ENOSR`, `ENOSTR`, `ETIME` - and neither does this. libc++ guards all four with `#ifdef` and
 * drops those `errc` enumerators when they are missing, which is the correct outcome: a number
 * invented here would be one no syscall on this console will ever return. The kernel's own
 * pseudo-errors (`ERESTART` and the negative values beside it) are likewise not userspace
...
/* The largest errno the platform defines, for a caller bounding a table by it. FreeBSD guards
 * this one behind `_POSIX_SOURCE`; nothing freestanding here defines that, so it is plain. */
#define ELAST           97
