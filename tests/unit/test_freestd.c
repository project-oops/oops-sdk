...
static void test_freestd_snprintf(void) {
    ...
    ret = oops_snprintf(buf, sizeof(buf), "Hello %s, num=%d, hex=0x%x", "world", 42,
                        0xabcd);
    ASSERT_EQ(ret, 31);
    ...
    ret = oops_snprintf(buf, sizeof(buf), "%llu|%p", 12345678901234ULL,
...
static void test_freestd_sets_and_tokens(void) {
...

    {
        ...
        t = obs_strtok_r(NULL, " ", &save);
        ASSERT_STR_EQ(t, "3.0");
        ASSERT_TRUE(obs_strtok_r(NULL, " ", &save) ==
                    NULL); /* the trailing space is not a token */
...
static void test_freestd_sscanf(void) {
    ...
    /* A face line: the literal slashes in the format have to match, and do not skip
     * whitespace. */
    {
        int v = 0, t = 0, nn = 0;
    ...
    /* A face line with no texture index - "7//3" - stops at the second slash, and the
     * count says so rather than leaving the caller to guess. */
    {
        int v = 0, t = -1, nn = -1;
     ...
     */
    {
        ...
        ASSERT_EQ(obs_sscanf("# a comment", "%d", &a),
                  0); /* a '#' is not a digit: no match */
        ASSERT_EQ(obs_sscanf("x1", "%d", &a), 0);
        ASSERT_EQ(obs_sscanf("", "%d", &a), -1); /* nothing at all: end of input */
        ASSERT_EQ(obs_sscanf("   ", "%d", &a),
                  -1); /* and whitespace alone is nothing at all */
        ASSERT_EQ(obs_sscanf("1 x", "%d %d", &a, &a),
                  1); /* one field, then a matching failure */
        ASSERT_EQ(a, 1);
    }
}
...
static int obs_scan_standard_answer(const char *in, const char *fmt, int *returns) {
    for (size_t i = 0; i < sizeof(k_host_diverges) / sizeof(k_host_diverges[0]); i++) {
        if (strcmp(in, k_host_diverges[i].in) == 0 &&
            strcmp(fmt, k_host_diverges[i].fmt) == 0) {
...
static void test_freestd_sscanf_agrees_with_the_host(void) {
    static const char *const inputs[] = {
        ...
        "00",       "\t7\t8\t",   "0x",
        "0xg",      "1e+",        "1e-5x",
        "-0",       "2147483648", "1.7976931348623157e308",
        "1e400",
    };
    for (size_t i = 0; i < sizeof(inputs) / sizeof(inputs[0]); i++) {
        ...
        }
        {
            ...
            if (obs_scan_standard_answer(in, "%f %f", &standard)) {
                ...
                ASSERT_EQ(mine, theirs);
                if (theirs >= 1)
                    ASSERT_FLOAT_NEAR(a, ha, 1e-5f);
                if (theirs >= 2)
                    ASSERT_FLOAT_NEAR(b, hb, 1e-5f);
            }
        }
        {
