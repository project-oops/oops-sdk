...
static void test_pkg_null_safety(void) {
  ...
  ASSERT_EQ(oops_pkg_get_progress("EP0001-CUSA00001_00-0000000000000000", NULL),
            -1);
  ASSERT_EQ(oops_pkg_get_progress_info(NULL, &info), -1);
  ASSERT_EQ(
      oops_pkg_get_progress_info("EP0001-CUSA00001_00-0000000000000000", NULL),
      -1);
}
...
static void test_pkg_unsupported_environment(void) {
  ...
  ASSERT_EQ(
      oops_pkg_get_progress("EP0001-CUSA00001_00-0000000000000000", &prog), -1);
  ASSERT_EQ(prog, 0u);
  ASSERT_EQ(
      oops_pkg_get_progress_info("EP0001-CUSA00001_00-0000000000000000", &info),
      -1);
