...
/* Unit tests for `oops/memory.h`. The host has no direct memory, so these pin the
 * constants, argument checks and honest refusal. */

/* The memory types and protection flags keep the platform's values. */
...
/* A zero-byte request returns NULL, and freeing NULL is safe. */
...
static void test_memory_alloc_refuses_wrapping_size(void) {
    ...
    ASSERT_TRUE(oops_mem_alloc(SIZE_MAX - 0x100, 0x10000, OOPS_MEM_WC_GARLIC) == NULL);
}
...
static void test_memory_direct_contract_on_host(void) {
    ...
    void *vaddr = NULL;
    uint8_t page[16];
    ASSERT_EQ(oops_mem_alloc_direct(0x10000, 0x10000, OOPS_MEM_WC_GARLIC, NULL), -1);
    ASSERT_EQ(oops_mem_alloc_direct(0, 0x10000, OOPS_MEM_WC_GARLIC, &phys), -1);
    ASSERT_EQ(oops_mem_alloc_direct(0x10000, 0x10000, OOPS_MEM_WC_GARLIC, &phys), -1);
    ASSERT_EQ(oops_mem_map_direct(&vaddr, 0x10000, OOPS_PROT_CPU_RW, 0, 0, 0x10000),
              -1);
    ASSERT_EQ(oops_mem_batch_map(NULL, 0, 0x10000, 0x4000, 0x33), -1);
    ASSERT_EQ(oops_mem_batch_map(page, 0, 0x10000, 0, 0x33), -1); /* zero page size */
    ASSERT_EQ(oops_mem_batch_map(page, 0, 0, 0x4000, 0x33), -1);  /* nothing to map */
    ASSERT_EQ(oops_mem_unmap(NULL, 0x10000), -1);
    ASSERT_TRUE(oops_mem_alloc(0x10000, 0x10000, OOPS_MEM_WC_GARLIC) == NULL);
}

/* Address-space reservation and release refuse bad arguments and fail on host. */
