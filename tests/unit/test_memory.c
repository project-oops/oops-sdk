...
/* A size that would wrap when rounded up to the page is refused, not rounded to something
 * small and handed back as if it fit. */
...
/* On a host with no platform, every direct call rejects rather than fabricating a mapping. */
static void test_memory_direct_contract_on_host(void) {
    ...
    ASSERT_EQ(oops_mem_map_direct(&vaddr, 0x10000, OOPS_PROT_CPU_RW, 0, 0, 0x10000), -1);
