...
/* A recording sink in place of the platform's output call: each call keeps one chunk. */
...
static int recording_sink(int handle, const void *chunk) {
    ...
    if (call == s_sink_fail_at) return -77;
    if (call < SINK_MAX_CALLS) {
        const int16_t *p = (const int16_t *)chunk;
        for (int i = 0; i < s_sink_chunk * OOPS_AUDIO_CHANNELS; i++) s_sink_log[call][i] = p[i];
    }
...
static void port_reset(int chunk_frames) {
    for (size_t i = 0; i < sizeof(s_port); i++) ((unsigned char *)&s_port)[i] = 0;
    s_port.handle = 7;
    ...
    for (int c = 0; c < SINK_MAX_CALLS; c++) {
        for (int i = 0; i < OOPS_AUDIO_MAX_CHUNK * OOPS_AUDIO_CHANNELS; i++) s_sink_log[c][i] = SENTINEL;
    }
}

/* Interleaved frames with a recognisable ramp: frame f carries (first + f, -(first + f)). */
...
/* Argument checks come before availability, so a host with no platform still reports a bad
 * call as bad; a good call is unavailable, never a fabricated port. */
...
/* Sizes that are not chunk multiples, written back to back, must join seamlessly: the old
 * path padded every call with silence, which is a click per write. */
