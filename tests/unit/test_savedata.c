...
static void test_savedata_null_safety(void) {
    char path[64];
    ASSERT_EQ(
        oops_savedata_mount(NULL, OOPS_SAVEDATA_MODE_READ_WRITE, path, sizeof(path)),
        -1);
    ASSERT_EQ(oops_savedata_mount("SAVE0000", OOPS_SAVEDATA_MODE_READ_WRITE, NULL,
                                  sizeof(path)),
              -1);
    ASSERT_EQ(oops_savedata_mount("SAVE0000", OOPS_SAVEDATA_MODE_READ_WRITE, path, 0),
              -1);
...
static void test_savedata_unsupported(void) {
    ...
    ASSERT_EQ(oops_savedata_mount("NONEXISTENT_SLOT_999", OOPS_SAVEDATA_MODE_READ_WRITE,
                                  path, sizeof(path)),
              -1);
...
static void test_savedata_filesystem_fallback(void) {
    ...
    ASSERT_EQ(
        oops_savedata_mount("UNITTEST_SLOT",
    ...
    char path2[128];
    ASSERT_EQ(oops_savedata_mount("UNITTEST_SLOT", OOPS_SAVEDATA_MODE_READ_ONLY, path2,
                                  sizeof(path2)),
              0);
...
static void test_savedata_file_helpers(void) {
    ...
    /* 1. Save file to slot */
    ASSERT_EQ(
        oops_savedata_save_file("SLOT_PREFS", "settings.bin", test_data, test_size), 0);

    /* 2. Load file back */
    void *loaded_data = NULL;
    size_t loaded_size = 0;
    ASSERT_EQ(oops_savedata_load_file("SLOT_PREFS", "settings.bin", &loaded_data,
                                      &loaded_size),
              0);
    ...
    void *fail_data = NULL;
    size_t fail_size = 0;
    ASSERT_NE(oops_savedata_load_file("SLOT_PREFS", "does_not_exist.bin", &fail_data,
                                      &fail_size),
              0);
}
