...
/* On a host with no platform symbols, audio decode and its offload engine are both
 * unavailable; the subsystem reports each honestly, rejects bad arguments, and never returns a
 * decoder while the control-struct layout is unconfirmed. */
...
static void test_audiodec_open_contract(void) {
    ...
    /* Valid codec on a host without the library: unavailable, not a fabricated handle. */
