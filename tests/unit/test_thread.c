...
/* With no platform threads on host: creation fails rather than returning a
 * handle, self is NULL, equality falls back to pointer comparison, and a timed
 * wait refuses rather than degrading into a wait that could never return. */
...
static void test_thread_exception_contract(void) {
  ...
  ASSERT_EQ(oops_thread_install_exception_handler(OOPS_EXCEPTION_SIGNAL, NULL),
            -1);
  ASSERT_EQ(oops_thread_install_exception_handler(OOPS_EXCEPTION_SIGNAL,
                                                  handler_probe),
            -1);
