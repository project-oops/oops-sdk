...
/* On a host with no platform symbols, video decode is unavailable; the subsystem must say so
 * honestly, reject bad arguments before any platform call, and never hand back a decoder while
 * the decode struct layouts are unconfirmed. */
...
static void test_videodec_open_contract(void) {
    ...
    /* Valid request on a host without the library: unavailable, not a fabricated handle. */
