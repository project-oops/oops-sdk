...
/* Unit tests for `oops/videodec.h`. On a host with no platform symbols, video decode
 * is unavailable, bad arguments are rejected before any platform call, and no decoder
 * is handed back. */

/* Video decode reports unavailable on host. */
static void test_videodec_unavailable_on_host(void) {
    ASSERT_EQ(oops_videodec_available(), 0);
}

/* Open rejects bad codecs and sizes as EPARAM and a valid request as EUNAVAIL. */
