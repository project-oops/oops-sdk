...
static void test_http_url_parse(void) {
  ...
  /* 1. Basic HTTP URL */
  ASSERT_EQ(oops_http_url_parse("http://example.com/test/index.html",
  ...
  /* 2. Custom Port */
  ASSERT_EQ(oops_http_url_parse("http://192.168.1.50:8080/api/apps.json",
                                scheme, sizeof(scheme),
                                host, sizeof(host),
                                &port, path, sizeof(path)), 0);
  ASSERT_STR_EQ(scheme, "http");
  ...
  ASSERT_EQ(oops_http_url_parse("https://api.github.com/repos/releases",
                                scheme, sizeof(scheme),
                                host, sizeof(host),
                                &port, path, sizeof(path)), 0);
  ...
  ASSERT_EQ(oops_http_url_parse("http://localhost",
                                scheme, sizeof(scheme),
                                host, sizeof(host),
                                &port, path, sizeof(path)), 0);
  ASSERT_STR_EQ(scheme, "http");
  ...
  /* 5. Invalid URLs */
  ASSERT_EQ(oops_http_url_parse(NULL, scheme, sizeof(scheme), host, sizeof(host), &port, path, sizeof(path)), -1);
  ASSERT_EQ(oops_http_url_parse("ftp://example.com/file", scheme, sizeof(scheme), host, sizeof(host), &port, path, sizeof(path)), -1);
  ASSERT_EQ(oops_http_url_parse("not_a_url", scheme, sizeof(scheme), host, sizeof(host), &port, path, sizeof(path)), -1);
  ASSERT_EQ(oops_http_url_parse("http://", scheme, sizeof(scheme), host, sizeof(host), &port, path, sizeof(path)), -1);
  ASSERT_EQ(oops_http_url_parse("http://host:999999/path", scheme, sizeof(scheme), host, sizeof(host), &port, path, sizeof(path)), -1);
}
...
static void test_http_build_request(void) {
  ...
  /* Buffer too small */
  char tiny[10];
  ASSERT_EQ(oops_http_build_request("example.com", "/index.html", tiny, sizeof(tiny)), -1);
}
...
static void test_http_parse_headers(void) {
  ...
  ASSERT_EQ(oops_http_parse_headers(resp1, strlen(resp1),
                                    &status, &content_len, &is_chunked,
                                    ct, sizeof(ct), loc, sizeof(loc), &hdr_end), 0);
  ...
  ASSERT_EQ(oops_http_parse_headers(resp2, strlen(resp2),
                                    &status, &content_len, &is_chunked,
                                    ct, sizeof(ct), loc, sizeof(loc), &hdr_end), 0);
  ...
  ASSERT_EQ(oops_http_parse_headers(resp3, strlen(resp3),
                                    &status, &content_len, &is_chunked,
                                    ct, sizeof(ct), loc, sizeof(loc), &hdr_end), 0);
  ASSERT_EQ(status, 200);
  ...
  ASSERT_EQ(oops_http_parse_headers(incomplete, strlen(incomplete),
                                    &status, &content_len, &is_chunked,
                                    ct, sizeof(ct), loc, sizeof(loc), &hdr_end), -1);
}
...
static void test_http_decode_chunked(void) {
  ...
  ASSERT_EQ(oops_http_decode_chunked(chunked, strlen(chunked), decoded, sizeof(decoded), &dec_len), 0);
  ASSERT_EQ(dec_len, 20);
  ASSERT_STR_EQ(decoded, "Wikipedia in chunks!");

  /* Malformed chunked stream */
  const char bad_chunk[] = "ZZZ\r\nBad chunk\r\n0\r\n\r\n";
  ASSERT_EQ(oops_http_decode_chunked(bad_chunk, strlen(bad_chunk), decoded, sizeof(decoded), &dec_len), -1);
}
...
static void test_http_api_contracts(void) {
  ...
  ASSERT_EQ(oops_http_get("https://example.com/test", &resp), OOPS_HTTP_ERR_TLS_UNAVAIL);
  ASSERT_EQ(oops_http_get_to_file("https://example.com/test", "/tmp/out"), OOPS_HTTP_ERR_TLS_UNAVAIL);
