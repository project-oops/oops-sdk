...
/* On host there is no pad, so init fails - and keeps failing. A second call
 * must report the same result, not a success because the first call left a flag
 * behind. */
...
/* Batched low-latency read: argument rejection, and on a host with no driver
 * the valid call is unavailable rather than a fabricated count. The stride is
 * pinned to the driver's 120-byte record by the static asserts in pad_layout.h.
 */
static void test_input_batch_bounds(void) {
    ...
    ASSERT_EQ(oops_input_poll_batch(0, batch, OOPS_MAX_PAD_SAMPLES),
              -1); /* valid, no driver */
}

/* Adaptive triggers are capture-gated: no false-positive availability, and the
 * effect call refuses every argument shape rather than pass a guessed struct.
 */
static void test_input_trigger_contract(void) {
    ...
    ASSERT_EQ(oops_input_set_trigger_effect(99, OOPS_TRIGGER_L2, OOPS_TRIGGER_FEEDBACK,
                                            0, 0, 0, 0),
              -1); /* bad port */
    ASSERT_EQ(oops_input_set_trigger_effect(0, 0, OOPS_TRIGGER_FEEDBACK, 0, 0, 0, 0),
              -1); /* empty trigger mask */
    ASSERT_EQ(oops_input_set_trigger_effect(0, OOPS_TRIGGER_L2, 99, 0, 0, 0, 0),
              -1); /* bad mode */
...
static void test_input_keyboard_contract(void) {
    ...
    /* Argument checks come before any platform question, so they answer the same either
     * way. */
    ...
    /* With no keyboard resolved, the button view agrees: no bits, and no crash reaching
     * for a record that was never filled. */
...
static void test_input_poll_keyboard_fold(void) {
    ...
    /* Ports above 0 never consult the keyboard, so one keypress cannot arrive four
     * times. */
    ASSERT_EQ(oops_input_poll(1, &st), -1);
    ASSERT_EQ((int)st.buttons, 0);

    /* The fold is switchable, and switching it does not change the no-device answer
     * either way. A program that reads characters turns it off so `a` is not also
     * GLUT_KEY_LEFT. */
...
/* The raw-record mapping, fed one record directly: this is where a wrong sign
 * or offset would otherwise first show on a controller. Uses the private layout
 * header. */
static void test_input_record_mapping(void) {
    ...
    for (size_t i = 0; i < sizeof(st); i++)
        ((unsigned char *)&st)[i] = 0xAA;

    raw.buttons = OOPS_BUTTON_CROSS | OOPS_BUTTON_L1;
    raw.leftStick.x = 0;
    raw.leftStick.y = 128;
    raw.rightStick.x = 255;
    raw.rightStick.y = 200;
    raw.analogButtons.l2 = 17;
    raw.analogButtons.r2 = 255;
    raw.quat.x = 0.1f;
    raw.quat.y = 0.2f;
    raw.quat.z = 0.3f;
    raw.quat.w = 0.4f;
    ...
    raw.touchData.touch[0].x = 1919;
    raw.touchData.touch[0].y = 941;
    raw.touchData.touch[0].finger = 7;
    raw.touchData.touch[1].x = 5;
    raw.touchData.touch[1].y = 6;
    raw.touchData.touch[1].finger = 8;
    raw.connected = 1;
    ...
    /* Connected follows the driver's flag alone: centred sticks read 128 and a
     * held button is not a pad, so neither may stand in for it. */
    raw.connected = 0;
