...
static void test_pipeline_draw_and_tile(void) {
    ...
    ASSERT_EQ(linear_fb[64 * width + 64], OOPS_COLOR_YELLOW); /* Diagonal intersects (64,64) */
    ASSERT_EQ(linear_fb[64 * width + 55], OOPS_COLOR_GREEN);  /* Off-diagonal inside circle */
    ...
    for (uint32_t i = 0; i < width * height; i++) {
        if (tiled_fb[i] != 0) non_zero_count++;
    }
