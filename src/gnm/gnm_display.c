...
sceVideoOutSetBufferAttribute(struct SceVideoOutBufferAttribute *attr,
...
__attribute__((weak)) int sceKernelCreateEqueue(sce_equeue_t *eq,
                                                const char *name);
__attribute__((weak)) int sceKernelDeleteEqueue(sce_equeue_t eq);
__attribute__((weak)) int sceKernelWaitEqueue(sce_equeue_t eq, void *ev,
                                              int num, int *out, void *tmo);
...
gnm_display_t *gnm_display_open(unsigned int width, unsigned int height) {
   ...
   * reported as it is. */
  if (width == 0 || height == 0 || width > GNM_MAX_DIM ||
      height > GNM_MAX_DIM) {
    disp->last_error = -2;
    return disp;
  }
  disp->fb_bytes =
      gnm_align_up((size_t)width * (size_t)height * 4u, GNM_FB_ALIGN);
  ...
  int arc = sceKernelAllocateDirectMemory(
      0, (sce_off_t)pool_size, disp->total_bytes, GNM_FB_ALIGN, 3, &physical);
  if (arc != 0) {
    disp->last_error = arc;
  ...
  void *mapped = 0;
  int mrc = sceKernelMapDirectMemory(&mapped, disp->total_bytes, 0x33, 0,
                                     physical, GNM_FB_ALIGN);
  if (mrc != 0 || !mapped) {
    disp->last_error = mrc;
  ...
  if (sceVideoOutSetBufferAttribute) {
    sceVideoOutSetBufferAttribute(&attr, 0x80000000u, 1 /* linear */,
                                  0 /* 16:9 */, width, height, width);
  }
...
int gnm_display_flip(gnm_display_t *disp) {
  ...
  int rc = sceVideoOutSubmitFlip(disp->handle, (int)shown, 1,
