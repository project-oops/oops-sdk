...
static inline size_t oops_surf_index(const oops_surface_t *s, uint32_t x, uint32_t y) {
...
void oops_draw_pixel(oops_surface_t *surf, int x, int y, oops_color_t color) {
    if (!surf || !surf->pixels)
        return;
    if (x < 0 || (uint32_t)x >= surf->width || y < 0 || (uint32_t)y >= surf->height)
        return;
...
void oops_draw_rect_blend(oops_surface_t *surf, int x, int y, int w, int h,
    ...
    for (int py = y0; py < y1; py++) {
        for (int px = x0; px < x1; px++) {
            uint32_t *p =
                &surf->pixels[oops_surf_index(surf, (uint32_t)px, (uint32_t)py)];
    ...
    for (int y = 0; y < sh; y++) {
        ...
        for (int x = 0; x < sw; x++) {
            dst->pixels[oops_surf_index(dst, (uint32_t)(dx + x), (uint32_t)(dy + y))] =
                src->pixels[oops_surf_index(src, (uint32_t)(sx + x),
                                            (uint32_t)(sy + y))];
        }
    }
}
...
void oops_draw_blit_blend(oops_surface_t *dst, int dx, int dy,
    ...
    for (int y = 0; y < sh; y++) {
        for (int x = 0; x < sw; x++) {
            const uint32_t s = src->pixels[oops_surf_index(src, (uint32_t)(sx + x),
                                                           (uint32_t)(sy + y))];
            uint32_t *p = &dst->pixels[oops_surf_index(dst, (uint32_t)(dx + x),
                                                       (uint32_t)(dy + y))];
            *p = oops_src_over(s, *p);
        }
    }
...
void oops_draw_blit_scaled_blend(oops_surface_t *dst, int dx, int dy, int dw, int dh,
                                 ...
                                 int sh) {
    if (!dst || !dst->pixels || !src || !src->pixels || sw <= 0 || sh <= 0 || dw <= 0 ||
        dh <= 0)
        return;
    ...
    for (int y = y0; y < y1; y++) {
        ...
        for (int x = x0; x < x1; x++) {
            ...
            uint32_t *p = &dst->pixels[oops_surf_index(dst, (uint32_t)x, (uint32_t)y)];
            *p = oops_src_over(s, *p);
        }
    }
}
