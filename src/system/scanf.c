...
/*
 * An integer, at most `width` characters. `base` 0 is C's own detection: `0x` for
 * sixteen, a leading zero for eight, otherwise ten. Returns 0 when no digit was read at
 * all, which is a matching failure and ends the whole scan.
 */
...
static int obs_scan_float(const char **pp, int width, double *out) {
    ...
    if (used + 1 < width && p[0] == '0' && (p[1] == 'x' || p[1] == 'X')) {
        ...
        if (used < width && (*p == 'p' || *p == 'P')) {
            ...
            if (edigits == 0) {
                return 0; /* a `p` with no digits fails the conversion, as `e` does
                             below */
            }
            if (ev > 1100) {
                ev = 1100;
...
/*
 * A character's membership of a `%[...]` scanset. `set` is the first character inside
 * the bracket and `end` the closing one. A leading `^` negates; a `]` immediately after
 * the bracket (or after the `^`) is a literal, as C says; `a-z` is a range, and a `-`
 * at either end of the set is itself.
 */
...
static void obs_scan_store_int(va_list *ap, int len, int is_signed,
                               unsigned long long v, int neg) {
...
int obs_vsscanf(const char *s, const char *fmt, va_list args) {
    ...
    for (; *fmt != '\0'; fmt++) {
        ...
        if (*fmt >= '0' && *fmt <= '9') {
            ...
            if (width == 0) {
                width = OBS_SCAN_NO_WIDTH; /* a width of zero is undefined; treat it as
                                              none */
            }
        }
        if (*fmt == 'h') {
...

        {
            ...
            case 'p':
                ...
                break;
            default:
                goto done; /* an unknown conversion ends the scan rather than guessing
                            */
            }
            unsigned long long v = 0;
            int neg = 0;
