...
__attribute__((weak)) int sceAppInstUtilAppExists(const char *titleId,
                                                  int32_t *exists);
__attribute__((weak)) int
sceAppInstUtilGetInstallProgress(const char *contentId, uint32_t *progress);
__attribute__((weak)) int
...
int oops_pkg_get_progress_info(const char *content_id,
  ...
  int rc = sceAppInstUtilGetInstallProgressInfo(
      content_id, &state, &prog, &prog_size, &tot_size, &rest_sec);
  if (rc == 0) {
