...
int oops_savedata_mount(const char *dir_name, oops_savedata_mode_t mode,
    ...
    if (!dir_name || !out_mount_path || max_path_len == 0)
        return -1;
    out_mount_path[0] = '\0';
    oops_log_info("SAVEDATA", "mount requested: dir='%s' mode=0x%x", dir_name,
                  (unsigned int)mode);
    ...
    if (s_savedata_initialized && s_pfn_sceSaveDataMount2) {
        ...
        *(uint32_t *)(mount_param + 64) = sizeof(mount_point);
        ...
        oops_log_debug("SAVEDATA",
                       "vendor mount failed rc=0x%x, falling back to title storage",
                       rc);
    }
    ...
    if (obs_strlen(slot_path) >= max_path_len) {
        oops_log_warn("SAVEDATA", "slot_path '%s' exceeds max_path_len %zu", slot_path,
                      max_path_len);
        return -1;
    }
    ...
    if (mode & OOPS_SAVEDATA_MODE_CREATE) {
        ...
        if (!oops_fs_exists(slot_path)) {
            oops_log_debug("SAVEDATA",
                           "directory '%s' does not exist (CREATE not specified)",
                           slot_path);
            return -1;
        }
    }
...
int oops_savedata_load_file(const char *dir_name, const char *file_name,
    ...
    char mount[256];
    if (oops_savedata_mount(dir_name, OOPS_SAVEDATA_MODE_READ_ONLY, mount,
                            sizeof(mount)) != 0)
        return -1;
...
int oops_savedata_save_file(const char *dir_name, const char *file_name,
    ...
    char mount[256];
    if (oops_savedata_mount(dir_name,
