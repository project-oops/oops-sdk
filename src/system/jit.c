...
__attribute__((weak)) int sceKernelJitCreateSharedMemory(const char *name,
                                                        size_t size, int flags,
                                                        int *handle);
__attribute__((weak)) int sceKernelJitMapSharedMemory(int handle, int flags,
                                                     void **addr);
__attribute__((weak)) int
sceKernelJitCreateAliasOfSharedMemory(int handle, int flags, void **addr);
__attribute__((weak)) int sceKernelMunmap(void *addr, size_t size);
__attribute__((weak)) int sceKernelClose(int fd);
__attribute__((weak)) void *mmap(void *addr, size_t len, int prot, int flags,
...
static int try_alloc_shared_mem(size_t aligned, oops_jit_memory_t *out_mem) {
  ...
  int handle = -1;
  int rc = sceKernelJitCreateSharedMemory("oops-jit", aligned, SCE_PROT_RWX,
                                         &handle);
  if (rc != 0 || handle < 0) {
    return -1;
  }
...
static int try_alloc_mprotect(size_t aligned, oops_jit_memory_t *out_mem) {
  ...
  if (mprotect) {
    ...
    mrc = (int)sys_call(SYS_mprotect, (long)ptr, (long)aligned, SCE_PROT_RWX,
                        0, 0, 0);
  }
  ...
  if (mem->method == OOPS_JIT_METHOD_SHARED_MEM) {
    if (mem->rx_addr) {
      if (sceKernelMunmap) {
        ...
        rc |= (int)sys_call(SYS_munmap, (long)mem->rx_addr, (long)mem->size, 0, 0, 0, 0);
      }
    }
    if (mem->rw_addr && mem->rw_addr != mem->rx_addr) {
      if (sceKernelMunmap) {
        ...
        rc |= (int)sys_call(SYS_munmap, (long)mem->rw_addr, (long)mem->size, 0, 0, 0, 0);
      }
    }
    if (mem->handle >= 0) {
      if (sceKernelClose) {
    ...
    if (addr) {
      if (munmap) {
        ...
        rc |= (int)sys_call(SYS_munmap, (long)addr, (long)mem->size, 0, 0, 0, 0);
      }
    }
  }
