...
__attribute__((weak)) int sceUserServiceGetUserName(int32_t userId, char *userName, size_t size);
...
__attribute__((weak)) int sceSystemServiceParamGetInt(int32_t paramId, int32_t *value);
__attribute__((weak)) int sceSysUtilSendSystemNotificationWithText(int type, const char *msg);
__attribute__((weak)) int sysctlbyname(const char *name, void *oldp, size_t *oldlenp, void *newp, size_t newlen);
...
__attribute__((weak)) int sceKernelGetSocSensorTemperature(int sensor, int *temperature);
...
int oops_user_get_name(int32_t user_id, char *out_name, size_t max_len) {
    if (!out_name || max_len == 0) return -1;
    out_name[0] = '\0';
...
int oops_user_get_logged_in_users(int32_t *out_user_ids, size_t max_users, size_t *out_count) {
    if (!out_user_ids || max_users == 0) return -1;
    ...
    for (size_t i = 0; i < sizeof(loginList); i++) ((unsigned char *)&loginList)[i] = 0;
...
int oops_system_get_info(oops_system_info_t *out_info) {
    ...
    if ((void *)&sceAgcDriverGetDefaultOwner != NULL) {
        ...
        out_info->generation = 4;
#else
        out_info->generation = 0; /* Unknown: could-not-look must never be reported as a generation */
#endif
    }
    ...
    if (out_info->initial_user_id >= 0) {
        oops_user_get_name(out_info->initial_user_id, out_info->user_name, sizeof(out_info->user_name));
    }
    ...
    if (sysctlbyname) {
        ...
        for (size_t i = 0; i < sizeof(version); i++) version[i] = 0;
        ...
        if (sysctlbyname("kern.version", version, &len, NULL, 0) == 0) {
            ...
            if (at != NULL) {
                size_t k = 0;
                while (at[k] != '\0' && at[k] != ' ' && k + 1 < sizeof(out_info->firmware_str)) {
                    out_info->firmware_str[k] = at[k];
                    k++;
                }
                out_info->firmware_str[k] = '\0';

                /* Parse numeric major.minor into firmware_raw (e.g. 12.40 -> 0x12400000) */
...
int oops_system_get_cpu_temp(int *out_temp_celsius) {
    if (!out_temp_celsius) return -1;
    *out_temp_celsius = -1;
    if (!sceKernelGetCpuTemperature) return -1;
...
int oops_system_get_cpu_freq(uint64_t *out_freq_hz) {
    if (!out_freq_hz) return -1;
    *out_freq_hz = 0;
    if (!sceKernelGetCpuFrequency) return -1;
...
int oops_system_get_hw_serial(char *out_serial, size_t max_len) {
    if (!out_serial || max_len == 0) return -1;
    out_serial[0] = '\0';
    if (!sceKernelGetHwSerialNumber) return -1;
    char buffer[1024];
...
int oops_system_get_hw_model(char *out_model, size_t max_len) {
    if (!out_model || max_len == 0) return -1;
    out_model[0] = '\0';
    if (!sceKernelGetHwModelName) return -1;
    char buffer[1024];
    for (size_t i = 0; i < sizeof(buffer); i++) buffer[i] = 0;
...
int oops_system_get_hw_info(oops_hw_info_t *out_hw) {
    ...
    (void)oops_system_get_hw_serial(out_hw->serial_number, sizeof(out_hw->serial_number));
    (void)oops_system_get_hw_model(out_hw->model_name, sizeof(out_hw->model_name));
