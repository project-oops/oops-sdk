...
uintptr_t loader_load_into_proc(pid_t pid, const uint8_t *elf_data,
  ...
  uintptr_t target_base =
      procctl_remote_mmap(pid, (ehdr->e_type == ET_EXEC) ? min_vaddr : 0,
                          total_size, PROC_PROT_READ | PROC_PROT_WRITE,
  ...
  if (ehdr->e_shoff != 0 && ehdr->e_shnum > 0 &&
      (ehdr->e_shoff + (uint64_t)ehdr->e_shnum * sizeof(Elf64_Shdr) <=
       elf_size)) {
    ...
    const Elf64_Sym *syms =
        (dynsym_sh != NULL)
            ? (const Elf64_Sym *)(elf_data + dynsym_sh->sh_offset)
            : NULL;
