...
static void utf8_to_utf16(const char *src, uint16_t *dst,
...
static void utf16_to_utf8(const uint16_t *src, char *dst,
                          size_t max_dst_bytes) {
  if (!dst || max_dst_bytes == 0)
    return;
  size_t i = 0;
  if (src) {
...
int oops_dialog_ime_open(const oops_ime_param_t *param) {
  ...
  /* Initialize buffers */
  for (size_t i = 0; i < IME_BUFFER_CHARS; i++)
    s_ime_text_buffer[i] = 0;
  ...
  for (size_t i = 0; i < sizeof(sce_param); i++)
    ((uint8_t *)&sce_param)[i] = 0;

  sce_param.userId = user_id;
  sce_param.type = (int32_t)param->type;
  sce_param.maxTextLength =
      param->max_text_len > 0 ? param->max_text_len : (IME_BUFFER_CHARS - 1);
...
oops_ime_status_t oops_dialog_ime_poll(void) {
  ...
  if (status == 1)
    return OOPS_IME_STATUS_RUNNING;
  if (status == 2)
    return OOPS_IME_STATUS_FINISHED;
  return OOPS_IME_STATUS_NONE;
}
...
int oops_dialog_ime_get_result(char *out_text, size_t max_len,
  ...
  if (rc == 0) {
    ...
    if (out_result) {
      if (res.endStatus == 0)
        *out_result = OOPS_IME_RESULT_OK;
      else if (res.endStatus == 1)
        *out_result = OOPS_IME_RESULT_CANCELED;
      else
        *out_result = OOPS_IME_RESULT_ABORTED;
    }
  }
  return rc;
}
...
static inline void
set_common_dialog_magic(sce_common_dialog_base_param_t *param) {
...
int oops_dialog_message_show(const char *message,
                             oops_msg_dialog_button_t buttons) {
  if (!message)
    return -1;
  ...
  for (size_t i = 0; i < sizeof(s_user_msg_param); i++)
    ((uint8_t *)&s_user_msg_param)[i] = 0;
  s_user_msg_param.buttonType = (int32_t)buttons;
  s_user_msg_param.msg = message;

  for (size_t i = 0; i < sizeof(s_msg_dialog_param); i++)
    ((uint8_t *)&s_msg_dialog_param)[i] = 0;
  ...
  if (status == 3 /* ORBIS_COMMON_DIALOG_STATUS_FINISHED */ || status == 2) {
    if (sceMsgDialogGetResult) {
      ...
      if (sceMsgDialogGetResult(&res) == 0) {
        if (out_result) {
          if (res.buttonId == 1)
            *out_result = OOPS_MSG_DIALOG_RES_OK;
          else if (res.buttonId == 2)
            *out_result = OOPS_MSG_DIALOG_RES_NO;
          else
