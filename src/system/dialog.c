...
int oops_dialog_ime_open(const oops_ime_param_t *param) {
    ...
    if (rc == 0) {
        s_ime_active = true;
        oops_log_info("DIALOG", "IME dialog opened successfully (user=%d, max_len=%u)",
                      user_id, param->max_text_len);
    } else {
    ...
    if (status == 3 /* ORBIS_COMMON_DIALOG_STATUS_FINISHED */ || status == 2) {
        if (sceMsgDialogGetResult) {
            ...
            if (sceMsgDialogGetResult(&res) == 0) {
                ...
                oops_log_debug("DIALOG", "message dialog finished (buttonId=%d)",
                               res.buttonId);
                return 1; /* Completed */
            }
        }
