...
int oops_inject_elf(pid_t target_pid, const uint8_t *payload_data,
                    size_t payload_size, payload_args_t *args) {
  ...
  uintptr_t entry_addr = loader_load_into_proc(
      target_pid, payload_data, payload_size, target_libkernel_base,
      &s_kexport_table, &target_base, &target_size);
  if (entry_addr == 0) {
  ...
  /* 7. Setup dedicated stack + payload_args + kexport table in target process
   * memory */
  uintptr_t alloc_remote = procctl_remote_mmap(
      target_pid, 0, 0x100000, PROC_PROT_READ | PROC_PROT_WRITE,
  ...
  size_t kexport_size = sizeof(uint32_t) * 2 +
                        sizeof(obs_kexport_entry_t) * s_kexport_table.count;
  ...
  uintptr_t tramp_remote = procctl_remote_mmap(
      target_pid, 0, 0x4000, PROC_PROT_READ | PROC_PROT_WRITE,
      PROC_MAP_ANONYMOUS | PROC_MAP_PRIVATE, -1, 0);
  ...
  procctl_remote_mprotect(target_pid, tramp_remote, 0x4000,
                          PROC_PROT_READ | PROC_PROT_EXEC);
  ...
  jmp_reg.r_rdi =
      args_remote; /* First argument (SysV ABI): payload_args_t *args */
