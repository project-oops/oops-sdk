...
__attribute__((weak)) int sceKernelReadProcessMemory(int pid, const void *addr,
                                                     void *buf, size_t size);
__attribute__((weak)) int sceKernelWriteProcessMemory(int pid, void *addr,
...
int oops_kernel_pipe_init(int rwpipe[2], int rwpair[2], uint64_t kpipe_addr,
    ...
    if (!rwpipe || !rwpair || kpipe_addr == 0)
        return -1;
    oops_log_info("ESCALATE",
                  "pipe_init rwpipe=[%d,%d] rwpair=[%d,%d] kpipe=0x%llx kdata=0x%llx",
...
int oops_kernel_rw_init(void) {
    ...
    if (sceKernelReadProcessMemory && sceKernelWriteProcessMemory &&
...
int oops_kernel_copyout(uint64_t kaddr, void *buf, size_t size) {
    ...
    if (sceKernelReadProcessMemory) {
        return sceKernelReadProcessMemory(-1, (const void *)(uintptr_t)kaddr, buf,
                                          size);
    }
...
uint64_t oops_kernel_find_proc_by_pid(int pid) {
    ...
    uint64_t allproc_addr =
        (allproc_off < 0x80000000ULL) ? (kbase + allproc_off) : allproc_off;
...
int oops_jailbreak_process(int pid) {
    ...
    (void)oops_kernel_copyin(&all_ones, ucred + common->cr_scecaps + 8,
                             sizeof(all_ones));
    ...
    if (fw && fw->rootvnode != 0) {
        ...
        uint64_t rootvnode_addr =
            (fw->rootvnode < 0x80000000ULL) ? (kbase + fw->rootvnode) : fw->rootvnode;
        uint64_t rootvnode_ptr = read_k_u64(rootvnode_addr);
        if (rootvnode_ptr != 0) {
            (void)oops_kernel_copyin(&rootvnode_ptr, fd + common->fd_rdir,
                                     sizeof(rootvnode_ptr));
            (void)oops_kernel_copyin(&rootvnode_ptr, fd + common->fd_jdir,
                                     sizeof(rootvnode_ptr));
        }
    }

    oops_log_info("ESCALATE", "jailbreak process pid=%d success (proc=0x%llx)", pid,
...
int oops_escalate_to_system_authid(void) {
    ...
    const oops_common_offsets_t *common = oops_offsets_get_common();
    if (!common) {
        oops_log_warn("ESCALATE",
                      "escalate_to_system_authid: common offsets not found");
        return -1;
    }
    ...
    if (rc == 0) {
        oops_log_info("ESCALATE", "escalated to system authid 0x%llx",
                      (unsigned long long)system_authid);
    } else {
...
int oops_has_system_authid(void) {
    ...
    uint64_t authid = 0;
    if (oops_kernel_copyout(ucred + common->cr_sceauthid, &authid, sizeof(authid)) !=
        0) {
        return -1;
    }
...
int oops_system_init_namespace(const struct payload_args *args) {
    ...
    if (args != NULL) {
        ...
        if (pa->rwpipe != NULL && pa->rwpair != NULL && pa->kpipe_addr != 0) {
            (void)oops_kernel_pipe_init(pa->rwpipe, pa->rwpair,
                                        (uint64_t)pa->kpipe_addr,
