...
sceKernelWriteProcessMemory(int pid, void *addr, const void *buf, size_t size);
...
int oops_jailbreak_process(int pid) {
  ...
  (void)oops_kernel_copyin(&authid, ucred + common->cr_sceauthid,
                           sizeof(authid));

  /* 3. Set capabilities (all-ones) */
  uint64_t all_ones = 0xFFFFFFFFFFFFFFFFULL;
  (void)oops_kernel_copyin(&all_ones, ucred + common->cr_scecaps,
                           sizeof(all_ones));
  ...
  if (fw && fw->rootvnode != 0) {
    ...
    uint64_t rootvnode_addr = (fw->rootvnode < 0x80000000ULL)
                                  ? (kbase + fw->rootvnode)
                                  : fw->rootvnode;
...
int oops_has_system_authid(void) {
  ...
  uint64_t authid = 0;
  if (oops_kernel_copyout(ucred + common->cr_sceauthid, &authid,
                          sizeof(authid)) != 0) {
    return -1;
  }
