 ...
 *
 * Left all-zero, this structure put `libSceLibcInternal` in its internal-memory fallback: a static
 * ~8-13 MiB heap that ignores the application's parameters. Mesa exhausted it - a title that brings
 * GL up through the DRI frontend crashed in `driConcatConfigs` on a `malloc` that returned NULL,
 * and a heap-ceiling probe could not even allocate one 8 MiB block (oops-mesa worklog 050).
 *
 * The fields below select **Application Heap Mode** (`mode = 0`, `version = 14`) and point libc at a
 * grow-on-demand heap: `heap_size = UINT64_MAX` (up to the container budget, ~432-448 MiB measured
 * on FW 12.40) and `extended_alloc = 1`. Every configuration entry is a *pointer* to its value, not
 * the value inline, populated by link-time relocations - which is why the values are separate
 * mutable statics. `init_alloc` is written to by libkernel during init, so it too is mutable.
 *
 * Layout, offsets, sentinels and the two trailing parameter blocks are the authoritative reading
 * of the 0xA8-byte `libc_param` from obSCEne REQ-20260919T1335Z-7b90 (retail `AgcCompositor.elf`
 * PT_SCE_PROCPARAM analysis, obscene#D298, and a measured 432-448 MiB budget). The old
 * "size-only, rest zeroed" form is what that request diagnosed as the fallback trigger.
 */
static uint64_t oops_libc_heap_size = 0xFFFFFFFFFFFFFFFFULL; /* grow to container capacity */
static uint32_t oops_libc_heap_extended_alloc = 1;           /* enable grow-on-demand */
static uint64_t oops_libc_heap_init_alloc = 0x80000;         /* 512 KiB; libkernel writes here */
...
_Static_assert(sizeof(oops_libc_param) == OOPS_LIBC_PARAM_SIZE, "oops_libc_param size mismatch");
