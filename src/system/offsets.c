...
int oops_offsets_load_string(const char *toml_str) {
    ...
    enum { SEC_NONE, SEC_COMMON, SEC_FW } cur_sec = SEC_NONE;
        ...
        if (*p != '\0' && *p != '#' && p < line_end) {
            ...
            if (*p == '[') {
                ...
                if (*p == ']') {
                    ...
                    if (len >= sizeof(sec_name))
                        len = sizeof(sec_name) - 1;
                    for (size_t k = 0; k < len; k++)
                        sec_name[k] = sec_start[k];
                    sec_name[len] = '\0';
                    ...
                    if (str_eq(sec_name, "common")) {
                        ...
                        if (s_fw_count < MAX_FW_ENTRIES) {
                            ...
                            str_copy(cur_fw->fw_version_str, sec_name,
                                     sizeof(cur_fw->fw_version_str));
                            cur_fw->fw_version_raw =
                                parse_fw_version_str_to_raw(sec_name);
                        } else {
                ...
                if (*eq == '=') {
                    ...
                    while (k_end > p && is_whitespace(*k_end))
                        k_end--;
                    ...
                    if (cur_sec == SEC_COMMON) {
                        if (str_eq(key, "p_ucred"))
                            s_common.p_ucred = (uint32_t)val;
                        else if (str_eq(key, "p_fd"))
                            s_common.p_fd = (uint32_t)val;
                        else if (str_eq(key, "p_pid"))
                            s_common.p_pid = (uint32_t)val;
                        else if (str_eq(key, "p_titleid"))
                            s_common.p_titleid = (uint32_t)val;
                        else if (str_eq(key, "p_comm"))
                            s_common.p_comm = (uint32_t)val;
                        else if (str_eq(key, "fd_rdir"))
                            s_common.fd_rdir = (uint32_t)val;
                        else if (str_eq(key, "fd_jdir"))
                            s_common.fd_jdir = (uint32_t)val;
                        else if (str_eq(key, "cr_uid"))
                            s_common.cr_uid = (uint32_t)val;
                        else if (str_eq(key, "cr_ruid"))
                            s_common.cr_ruid = (uint32_t)val;
                        else if (str_eq(key, "cr_svuid"))
                            s_common.cr_svuid = (uint32_t)val;
                        else if (str_eq(key, "cr_ngroups"))
                            s_common.cr_ngroups = (uint32_t)val;
                        else if (str_eq(key, "cr_rgid"))
                            s_common.cr_rgid = (uint32_t)val;
                        else if (str_eq(key, "cr_svgid"))
                            s_common.cr_svgid = (uint32_t)val;
                        else if (str_eq(key, "cr_prison"))
                            s_common.cr_prison = (uint32_t)val;
                        else if (str_eq(key, "cr_sceauthid"))
                            s_common.cr_sceauthid = (uint32_t)val;
                        else if (str_eq(key, "cr_scecaps"))
                            s_common.cr_scecaps = (uint32_t)val;
                        else if (str_eq(key, "cr_sceattrs"))
                            s_common.cr_sceattrs = (uint32_t)val;
                        else if (str_eq(key, "cr_sceattr0"))
                            s_common.cr_sceattr0 = (uint32_t)val;
                        else if (str_eq(key, "pr_ref"))
                            s_common.pr_ref = (uint32_t)val;
                    } else if (cur_sec == SEC_FW && cur_fw) {
                        if (str_eq(key, "kdata_base"))
                            cur_fw->kdata_base = val;
                        else if (str_eq(key, "allproc"))
                            cur_fw->allproc = val;
                        else if (str_eq(key, "rootvnode"))
                            cur_fw->rootvnode = val;
                        else if (str_eq(key, "security_flags"))
                            cur_fw->security_flags = val;
                        else if (str_eq(key, "qa_flags"))
                            cur_fw->qa_flags = val;
                        else if (str_eq(key, "utoken_flags"))
                            cur_fw->utoken_flags = val;
                        else if (str_eq(key, "sysents"))
                            cur_fw->sysents = val;
                        else if (str_eq(key, "sysentvec"))
                            cur_fw->sysentvec = val;
                    }
                }
            }
        }
...
int oops_offsets_load_default(void) {
    static const char *search_paths[] = {"/app0/offsets.toml", "/data/offsets.toml",
                                         "./offsets.toml", "data/offsets.toml",
                                         "../data/offsets.toml"};
