...
static long find_libkernel_syscall_gadget(void) {
    ...
    /* Prospero: libkernel is mapped Execute-Only (XO). Dereferencing causes
     * SYSTEM_XO_VIOLATION. getpid is at 0x4e0; getpid + 0xa (0x4ea) is 'syscall; jb +1;
     * ret'. */
...
#ifndef OOPS_HOST_BUILD
    if (uargs == ~0UL) {
        __attribute__((weak)) long sceKernelWrite(int fd, const void *buf,
                                                  unsigned long len);
