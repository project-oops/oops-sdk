 ...
 *
 * Text uses the built-in bitmap font scaled by size/8, and its width is measured to
 * match exactly what draw_text renders (both quantise to 8*scale per glyph). Radial and
 * conic gradients fill with their first colour, linear ones with their end colours.
 * Images are fetched over HTTP and PNG-decoded into `m_image_cache` by `load_image`;
 * clip rectangles are tracked but not applied.
 */
...
litehtml::pixel_t oops_container::text_width(const char *text,
    ...
    // Mirror draw_text exactly. The 8x8 bitmap font is drawn at scale = max(1, size/8)
    // and advances 8*scale pixels per character (draw.c: `cur_x += FONT_WIDTH * scale`,
    // FONT_WIDTH == 8). Measuring with any other metric (the old 0.6*size) makes glyphs
    // render wider than their measured box, so the following inline box overlaps them -
    // visible as the 30px brand drawing "index" on top of "oops-apps". Quantising the
    // width the same way keeps measurement and drawing in lockstep.
    int scale = size / 8;
    if (scale < 1)
        scale = 1;
    int char_w = 8 * scale;
...
void oops_container::load_image(const char *src, const char *baseurl,
    ...
    // The page uses absolute https URLs for icons; fall back to base+src for anything
    // relative.
    std::string url;
    ...
    // An empty surface is cached whatever happens, so a failed fetch is attempted once,
    // not on every re-render. Decode straight to a fixed square; draw_image resamples
    // to the element box.
    ...
    if (rc == OOPS_HTTP_OK && resp.status_code == 200 && resp.body &&
        ...
        if (pix) {
            ...
            if (dr == 0) {
                ...
                cached.height = TH;
                cached.pitch = TW;
                oops_kprintf_level(OOPS_LOG_INFO, "HTML",
                                   "load_image ok %s (%ux%u->%ux%u)", url.c_str(), ow,
                                   oh, TW, TH);
            } else {
                std::free(pix);
                oops_kprintf_level(OOPS_LOG_ERROR, "HTML",
                                   "load_image decode failed rc=%d %s", dr,
                                   url.c_str());
            }
        }
    } else {
        oops_kprintf_level(OOPS_LOG_ERROR, "HTML",
                           "load_image fetch failed rc=%d status=%d %s", rc,
                           rc == OOPS_HTTP_OK ? resp.status_code : -1, url.c_str());
    }
    if (rc == OOPS_HTTP_OK)
...
// Scroll vertically the minimum needed so the document range [top, top+h] is inside the
// viewport. A keyboard/pad-driven page has no mouse wheel, so it moves a highlight and
// asks the view to keep the highlighted element on screen.
