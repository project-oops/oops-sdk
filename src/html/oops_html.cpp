...
void oops_html_set_size(oops_html_t *html, int width, int height) {
    if (!html || !html->container || width <= 0 || height <= 0) return;
...
int oops_html_load(oops_html_t *html, const char *html_source, const char *base_url) {
    ...
    oops_log_debug("HTML", "Parsing HTML document (length: %zu bytes)", strlen(html_source));
    auto doc = litehtml::document::createFromString(html_source, html->container.get(), litehtml::master_css);
    if (!doc) {
...
void *oops_html_get_document(oops_html_t *html) {
    if (!html || !html->container || !html->container->m_doc) return NULL;
    return html->container->m_doc.get();
}

void *oops_html_get_container(oops_html_t *html) {
    if (!html || !html->container) return NULL;
