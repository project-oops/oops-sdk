...
void gluPickMatrix(GLdouble x, GLdouble y, GLdouble dx, GLdouble dy, GLint *viewport) {
    if (!viewport || dx <= 0.0 || dy <= 0.0)
        return;
    glTranslatef(
        (GLfloat)(((GLdouble)viewport[2] - 2.0 * (x - (GLdouble)viewport[0])) / dx),
        (GLfloat)(((GLdouble)viewport[3] - 2.0 * (y - (GLdouble)viewport[1])) / dy),
        0.0f);
    glScalef((GLfloat)((GLdouble)viewport[2] / dx),
             (GLfloat)((GLdouble)viewport[3] / dy), 1.0f);
}
...
static GLboolean glu_invert(const GLdouble *m, GLdouble *inv) {
    ...
    for (int i = 0; i < 16; i++)
        a[i] = m[i];
    GLdouble c[16];
    c[0] = a[5] * a[10] * a[15] - a[5] * a[11] * a[14] - a[9] * a[6] * a[15] +
           a[9] * a[7] * a[14] + a[13] * a[6] * a[11] - a[13] * a[7] * a[10];
    c[4] = -a[4] * a[10] * a[15] + a[4] * a[11] * a[14] + a[8] * a[6] * a[15] -
           a[8] * a[7] * a[14] - a[12] * a[6] * a[11] + a[12] * a[7] * a[10];
    c[8] = a[4] * a[9] * a[15] - a[4] * a[11] * a[13] - a[8] * a[5] * a[15] +
           a[8] * a[7] * a[13] + a[12] * a[5] * a[11] - a[12] * a[7] * a[9];
    c[12] = -a[4] * a[9] * a[14] + a[4] * a[10] * a[13] + a[8] * a[5] * a[14] -
            a[8] * a[6] * a[13] - a[12] * a[5] * a[10] + a[12] * a[6] * a[9];
    c[1] = -a[1] * a[10] * a[15] + a[1] * a[11] * a[14] + a[9] * a[2] * a[15] -
           a[9] * a[3] * a[14] - a[13] * a[2] * a[11] + a[13] * a[3] * a[10];
    c[5] = a[0] * a[10] * a[15] - a[0] * a[11] * a[14] - a[8] * a[2] * a[15] +
           a[8] * a[3] * a[14] + a[12] * a[2] * a[11] - a[12] * a[3] * a[10];
    c[9] = -a[0] * a[9] * a[15] + a[0] * a[11] * a[13] + a[8] * a[1] * a[15] -
           a[8] * a[3] * a[13] - a[12] * a[1] * a[11] + a[12] * a[3] * a[9];
    c[13] = a[0] * a[9] * a[14] - a[0] * a[10] * a[13] - a[8] * a[1] * a[14] +
            a[8] * a[2] * a[13] + a[12] * a[1] * a[10] - a[12] * a[2] * a[9];
    c[2] = a[1] * a[6] * a[15] - a[1] * a[7] * a[14] - a[5] * a[2] * a[15] +
           a[5] * a[3] * a[14] + a[13] * a[2] * a[7] - a[13] * a[3] * a[6];
    c[6] = -a[0] * a[6] * a[15] + a[0] * a[7] * a[14] + a[4] * a[2] * a[15] -
           a[4] * a[3] * a[14] - a[12] * a[2] * a[7] + a[12] * a[3] * a[6];
    c[10] = a[0] * a[5] * a[15] - a[0] * a[7] * a[13] - a[4] * a[1] * a[15] +
            a[4] * a[3] * a[13] + a[12] * a[1] * a[7] - a[12] * a[3] * a[5];
    c[14] = -a[0] * a[5] * a[14] + a[0] * a[6] * a[13] + a[4] * a[1] * a[14] -
            a[4] * a[2] * a[13] - a[12] * a[1] * a[6] + a[12] * a[2] * a[5];
    c[3] = -a[1] * a[6] * a[11] + a[1] * a[7] * a[10] + a[5] * a[2] * a[11] -
           a[5] * a[3] * a[10] - a[9] * a[2] * a[7] + a[9] * a[3] * a[6];
    c[7] = a[0] * a[6] * a[11] - a[0] * a[7] * a[10] - a[4] * a[2] * a[11] +
           a[4] * a[3] * a[10] + a[8] * a[2] * a[7] - a[8] * a[3] * a[6];
    c[11] = -a[0] * a[5] * a[11] + a[0] * a[7] * a[9] + a[4] * a[1] * a[11] -
            a[4] * a[3] * a[9] - a[8] * a[1] * a[7] + a[8] * a[3] * a[5];
    c[15] = a[0] * a[5] * a[10] - a[0] * a[6] * a[9] - a[4] * a[1] * a[10] +
...
GLint gluProject(GLdouble objX, GLdouble objY, GLdouble objZ, const GLdouble *model,
    ...
    if (!model || !proj || !view || !winX || !winY || !winZ)
        return GL_FALSE;
...
void gluQuadricDrawStyle(GLUquadric *q, GLenum draw) {
    if (!q)
        return;
    if (draw != GLU_FILL && draw != GLU_LINE && draw != GLU_SILHOUETTE &&
        draw != GLU_POINT) {
...
void gluQuadricTexture(GLUquadric *q, GLboolean texture) {
    if (q)
        q->texture = (GLboolean)(texture ? GL_TRUE : GL_FALSE);
}
...
void gluSphere(GLUquadric *q, GLdouble radius, GLint slices, GLint stacks) {
    ...
    for (GLint cap = 0; cap < 2 && stacks >= 2; cap++) {
        ...
        const float t_ring =
            north ? (1.0f - 1.0f / (float)stacks) : (1.0f / (float)stacks);
        const GLboolean backwards =
            (GLboolean)(north != (GLboolean)(inside != GL_FALSE));
        glBegin(mode == GL_QUAD_STRIP ? GL_TRIANGLE_FAN : mode);
        glu_normal(q, 0.0f, 0.0f, pz);
        if (q->texture)
            glTexCoord2f(0.5f, t_pole);
        ...
        for (GLint k = 0; k <= slices; k++) {
            ...
            if (q->texture)
                glTexCoord2f((float)j / (float)slices, t_ring);
    ...
    if (q->draw_style == GLU_LINE) {
        for (GLint j = 0; j < slices; j++) {
            ...
            for (GLint i = 0; i <= stacks; i++) {
                ...
                if (q->texture) {
                    glTexCoord2f((float)j / (float)slices,
...
void gluCylinder(GLUquadric *q, GLdouble base, GLdouble top, GLdouble height,
    ...
    for (GLint i = 0; i < stacks; i++) {
        const float z0 = hh * (float)i / (float)stacks,
                    z1 = hh * (float)(i + 1) / (float)stacks;
    ...
    if (q->draw_style == GLU_LINE || q->draw_style == GLU_SILHOUETTE) {
        for (GLint j = 0; j < slices; j++) {
            ...
            glu_normal(q, st * nr, ct * nr, nz);
            if (q->texture)
                glTexCoord2f((float)j / (float)slices, 0.0f);
            glVertex3f(st * rb, ct * rb, 0.0f);
            if (q->texture)
                glTexCoord2f((float)j / (float)slices, 1.0f);
...
static void glu_disk(GLUquadric *q, GLdouble inner, GLdouble outer, GLint slices,
    ...
    const float a0 = (float)start * (GLU_PI / 180.0f),
                da = (float)sweep * (GLU_PI / 180.0f);
    ...
    for (GLint i = 0; i < loops; i++) {
        ...
        for (GLint j = 0; j <= slices; j++) {
            ...
            for (int e = 0; e < 2; e++) {
                ...
                if (q->texture)
                    glTexCoord2f(x / ro * 0.5f + 0.5f, y / ro * 0.5f + 0.5f);
                glVertex3f(x, y, 0.0f);
            }
        }
        glEnd();
...
static void glu_unpack_pixel(GLenum format, GLenum type, const uint8_t *src,
                             float rgba[4]) {
    rgba[0] = 0.0f;
    rgba[1] = 0.0f;
    rgba[2] = 0.0f;
...
static void glu_pack_pixel(GLenum format, GLenum type, const float rgba[4],
                           uint8_t *dst) {
    if (type == GL_UNSIGNED_BYTE) {
...
static void glu_box_pixel(GLenum format, GLenum type, const uint8_t *src,
                          size_t src_row, size_t pixel_bytes, GLsizei w_in,
                          GLsizei h_in, double x0, double x1, double y0, double y1,
                          float out[4]) {
...
static void glu_box_scale(GLenum formatIn, GLenum typeIn, const uint8_t *src,
                          size_t src_row, GLsizei w_in, GLsizei h_in, GLenum formatOut,
                          GLenum typeOut, uint8_t *dst, size_t dst_row, GLsizei w_out,
                          GLsizei h_out) {
    ...
    for (GLsizei y = 0; y < h_out; y++) {
        ...
        for (GLsizei x = 0; x < w_out; x++) {
            ...
            glu_box_pixel(formatIn, typeIn, src, src_row, in_bytes, w_in, h_in,
                          (double)x * sx, (double)(x + 1) * sx, (double)y * sy,
                          (double)(y + 1) * sy, rgba);
...
GLint gluScaleImage(GLenum format, GLsizei wIn, GLsizei hIn, GLenum typeIn,
    ...
    GLint unpack_row_length = 0, unpack_alignment = 4, unpack_skip_rows = 0,
          unpack_skip_pixels = 0;
    GLint pack_row_length = 0, pack_alignment = 4, pack_skip_rows = 0,
          pack_skip_pixels = 0;
    ...
    in_stride =
        (in_stride + (size_t)unpack_alignment - 1u) & ~((size_t)unpack_alignment - 1u);

    size_t out_width = (pack_row_length > 0) ? (size_t)pack_row_length : (size_t)wOut;
    size_t out_stride = out_width * (size_t)out_bytes;
    out_stride =
        (out_stride + (size_t)pack_alignment - 1u) & ~((size_t)pack_alignment - 1u);

    const uint8_t *src = (const uint8_t *)dataIn +
                         (size_t)unpack_skip_rows * in_stride +
                         (size_t)unpack_skip_pixels * (size_t)in_bytes;
    uint8_t *dst = (uint8_t *)dataOut + (size_t)pack_skip_rows * out_stride +
                   (size_t)pack_skip_pixels * (size_t)out_bytes;

    glu_box_scale(format, typeIn, src, in_stride, wIn, hIn, format, typeOut, dst,
                  out_stride, wOut, hOut);
    return 0;
}
...
static GLint glu_build_mipmaps(GLenum target, GLint internalFormat, GLsizei width,
                               GLsizei height, GLenum format, GLenum type,
                               const void *data, GLboolean one_d) {
    if (width <= 0 || height <= 0)
        return GLU_INVALID_VALUE;
    if (!data)
        return GLU_INVALID_VALUE;
    ...
    size_t in_width =
        (unpack_row_length > 0) ? (size_t)unpack_row_length : (size_t)width;
    size_t in_stride = in_width * (size_t)pixel_bytes;
    in_stride =
        (in_stride + (size_t)unpack_alignment - 1u) & ~((size_t)unpack_alignment - 1u);
    const uint8_t *src = (const uint8_t *)data + (size_t)unpack_skip_rows * in_stride +
                         (size_t)unpack_skip_pixels * (size_t)pixel_bytes;

    glu_box_scale(format, type, src, in_stride, width, height, format, type, level,
                  (size_t)w * (size_t)pixel_bytes, w, h);

    /* Switch unpack state to tightly packed 1-byte aligned for our internal mipmap
     * buffers */
    ...
    for (;;) {
        ...
        uint8_t *next =
            (uint8_t *)glu_alloc((size_t)nw * (size_t)nh * (size_t)pixel_bytes);
        if (!next) {
            glu_free(level);
...
GLint gluBuild2DMipmaps(GLenum target, GLint internalFormat, GLsizei width,
    ...
    return glu_build_mipmaps(target, internalFormat, width, height, format, type, data,
                             GL_FALSE);
}

GLint gluBuild1DMipmaps(GLenum target, GLint internalFormat, GLsizei width,
                        GLenum format, GLenum type, const void *data) {
    return glu_build_mipmaps(target, internalFormat, width, 1, format, type, data,
                             GL_TRUE);
}
