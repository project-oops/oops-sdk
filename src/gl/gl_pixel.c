...
GLenum gl_pixel_fmt(GLenum format, GLenum type, gl_pixel_fmt_t *out) {
    ...
    if (type == GL_BITMAP) {
        ...
        out->type = type;
        out->kind =
            (format == GL_COLOR_INDEX) ? (GLenum)GL_COLOR_INDEX : (GLenum)GL_STENCIL;
        out->components = 1;
        out->packed = GL_FALSE;
        out->bitmap = GL_TRUE;
        out->elem_bytes = 0u;
    ...
    const GLboolean value =
        (GLboolean)(format == GL_DEPTH_COMPONENT || format == GL_STENCIL_INDEX ||
                    format == GL_COLOR_INDEX);
    /* A depth, stencil or colour-index value is one number, which no packed type holds
     * (Mesa main/glformats.c, _mesa_error_check_format_and_type). */
    if (pk && value)
        return GL_INVALID_OPERATION;
    if (pk) {
        /* Which formats each packed type fits: three-field ones RGB (and BGR for 5-6-5,
         * which Mesa accepts), four-field ones RGBA or BGRA. */
        GLboolean ok;
        if (pk->n == 3) {
    ...
    out->kind = (format == GL_DEPTH_COMPONENT) ? (GLenum)GL_DEPTH
                ...
                : (format == GL_COLOR_INDEX)   ? (GLenum)GL_COLOR_INDEX
                                               : (GLenum)GL_COLOR;
    out->components = n;
...
void gl_unpack_source(const gl_context_t *ctx, const gl_pixel_fmt_t *f,
    ...
    const size_t rows = ctx->unpack_image_height > 0 ? (size_t)ctx->unpack_image_height
                                                     : (size_t)height;
    /* A bitmap's rows are bytes of bits; its skipped pixels are bits, applied per pixel
     * by gl_unpack_bit rather than here. */
    if (f->bitmap) {
...
void gl_pack_dest(const gl_context_t *ctx, const gl_pixel_fmt_t *f, void *pixels,
    ...
    const size_t row_px =
        ctx->pack_row_length > 0 ? (size_t)ctx->pack_row_length : (size_t)width;
    const size_t rows =
        ctx->pack_image_height > 0 ? (size_t)ctx->pack_image_height : (size_t)height;
    if (f->bitmap) {
...
static float gl_elem_to_f(GLenum type, uint32_t v) {
    ...
    case GL_BYTE:
        return (float)((2.0 * (double)(int8_t)(uint8_t)v + 1.0) / 255.0);
    case GL_UNSIGNED_SHORT:
        return (float)v / 65535.0f;
    case GL_SHORT:
        return (float)((2.0 * (double)(int16_t)(uint16_t)v + 1.0) / 65535.0);
    case GL_UNSIGNED_INT:
        return (float)((double)v / 4294967295.0);
    case GL_INT:
        return (float)((2.0 * (double)(int32_t)v + 1.0) / 4294967295.0);
    case GL_FLOAT: {
        float f;
        memcpy(&f, &v, 4);
        return f;
...
/* To the nearest integer, a tie going toward zero: 0 packs as ((2^b - 1)0 - 1) / 2 =
 * -1/2, and Mesa's FLOAT_TO_BYTE / FLOAT_TO_SHORT (main/macros.h:59, :83) make that 0
 * by integer division. */
...
/* A fraction of one as an element of a plain type - the inverse of gl_elem_to_f,
 * rounded: unsigned c(2^b - 1), signed ((2^b - 1)c - 1) / 2 (the specification's pack
 * conversions). */
static uint32_t gl_f_to_elem(GLenum type, float c) {
    const double d = (double)c;
    switch (type) {
    case GL_UNSIGNED_BYTE:
        return gl_round_u32(gl_clampd(d, 0.0, 1.0) * 255.0);
    case GL_UNSIGNED_SHORT:
        return gl_round_u32(gl_clampd(d, 0.0, 1.0) * 65535.0);
    case GL_UNSIGNED_INT:
        return gl_round_u32(gl_clampd(d, 0.0, 1.0) * 4294967295.0);
    case GL_BYTE: {
        const int64_t r =
            gl_round_signed((gl_clampd(d, -1.0, 1.0) * 255.0 - 1.0) * 0.5);
        return (uint32_t)(uint8_t)(int8_t)(r < -128 ? -128 : (r > 127 ? 127 : r));
    }
    case GL_SHORT: {
        const int64_t r =
            gl_round_signed((gl_clampd(d, -1.0, 1.0) * 65535.0 - 1.0) * 0.5);
        return (uint32_t)(uint16_t)(int16_t)(r < -32768 ? -32768
                                                        : (r > 32767 ? 32767 : r));
    }
    case GL_INT: {
        const int64_t r =
            gl_round_signed((gl_clampd(d, -1.0, 1.0) * 4294967295.0 - 1.0) * 0.5);
        return (uint32_t)(int32_t)(r < -2147483648LL
                                       ? -2147483648LL
                                       : (r > 2147483647LL ? 2147483647LL : r));
    }
    case GL_FLOAT: {
        uint32_t v;
        memcpy(&v, &c, 4);
...
void gl_unpack_pixel_f(const gl_pixel_fmt_t *f, const uint8_t *src, GLboolean swap,
    ...
    if (f->packed) {
        ...
        for (int i = 0; i < pk->n; i++)
            v[i] = (float)((e >> shift[i]) & mask[i]) / (float)mask[i];
    } else {
...
void gl_pack_pixel_f(const gl_pixel_fmt_t *f, const float rgba[4], GLboolean lum_sum,
    ...
    if (f->packed) {
        ...
        for (int i = 0; i < pk->n; i++) {
            const uint32_t q =
                gl_round_u32(gl_clampd((double)v[i], 0.0, 1.0) * (double)mask[i]);
            e |= (q & mask[i]) << shift[i];
        }
    ...
    for (int i = 0; i < f->components; i++) {
        gl_write_elem(dst + (size_t)i * f->elem_bytes, f->elem_bytes,
                      gl_f_to_elem(f->type, v[i]), swap);
    }
}
...
void gl_tex_store_row(const gl_context_t *ctx, const gl_pixel_fmt_t *f, uint8_t *dst,
    ...
    if (base == GL_DEPTH_COMPONENT) {
        /* A depth texel is the 32-bit float itself, in the four bytes a colour texel
         * takes. */
...
void gl_unpack_row(const gl_context_t *ctx, const gl_pixel_fmt_t *f, uint8_t *dst,
    ...
    for (size_t x = 0; x < (size_t)width; x++) {
        ...
        if (f->kind == GL_COLOR_INDEX) {
            /* An index becomes RGBA through the I_TO_* maps, which stand in for the
             * RGBA transfer - so that is not applied as well. */
 ...
 *
 * GL_DEPTH_COMPONENT and GL_STENCIL_INDEX pixels are one number each. A depth value
 * converts as a colour component does - an integer is a fraction of one - and goes
 * through GL_DEPTH_SCALE and GL_DEPTH_BIAS and a clamp to [0, 1]; a stencil index is
 * the integer itself, shifted and offset by GL_INDEX_SHIFT and GL_INDEX_OFFSET and
 * looked up in GL_PIXEL_MAP_S_TO_S under GL_MAP_STENCIL (GL 1.x, 3.6.5 and 4.3.2). The
 * transfer is skipped while suspended, as a colour's is.
 * ------------------------------------------------------------------------- */

/* Mesa's order (main/pixeltransfer.c:220-252, _mesa_apply_stencil_transfer_ops): shift
 * - left for a positive GL_INDEX_SHIFT, right for a negative - and offset, then the
 * S_TO_S map indexed by the value masked to the map's size, which is a power of two. */
static int64_t gl_index_shift_offset(const gl_context_t *ctx, int64_t s) {
    if (ctx->index_shift > 0)
        s = (int64_t)((uint64_t)s << ctx->index_shift);
    else if (ctx->index_shift < 0)
        s >>= -ctx->index_shift;
...
static int64_t gl_index_elem(GLenum type, uint32_t e) {
    switch (type) {
    case GL_BYTE:
        return (int8_t)(uint8_t)e;
    case GL_SHORT:
        return (int16_t)(uint16_t)e;
    case GL_INT:
        return (int32_t)e;
    case GL_FLOAT: {
        float v;
...
GLboolean gl_unpack_bit(const gl_context_t *ctx, const uint8_t *row, int x) {
    ...
    const unsigned shift =
        ctx->unpack_lsb_first ? (unsigned)(bit % 8u) : 7u - (unsigned)(bit % 8u);
...
void gl_pack_bit(const gl_context_t *ctx, uint8_t *row, int x, GLboolean v) {
    ...
    if (v)
        row[bit / 8u] = (uint8_t)(row[bit / 8u] | (1u << shift));
    else
        row[bit / 8u] = (uint8_t)(row[bit / 8u] & ~(1u << shift));
}

void gl_index_to_rgba(const gl_context_t *ctx, int64_t i, float out[4]) {
    if (!ctx->pixel_transfer_suspend)
        i = gl_index_shift_offset(ctx, i);
    for (int c = 0; c < 4; c++) {
...
static int64_t gl_row_index(const gl_context_t *ctx, const gl_pixel_fmt_t *f,
                            const uint8_t *row, int x, GLboolean swap) {
    if (f->bitmap)
        return gl_unpack_bit(ctx, row, x) ? 1 : 0;
    return gl_index_elem(
        f->type, gl_read_elem(row + (size_t)x * f->pixel_bytes, f->elem_bytes, swap));
}
...
float gl_unpack_value(const gl_context_t *ctx, const gl_pixel_fmt_t *f,
    ...
    if (f->kind == GL_DEPTH) {
        const uint32_t e =
            gl_read_elem(row + (size_t)x * f->pixel_bytes, f->elem_bytes, swap);
        float d = gl_elem_to_f(f->type, e);
        if (transfer)
            d = d * ctx->depth_scale + ctx->depth_bias;
        return !(d > 0.0f) ? 0.0f : ((d > 1.0f) ? 1.0f : d);
    }
    /* A stencil index: the element's integer value, a float truncated to one - or its
     * bit. */
...
void *gl_pixel_copy_client(const gl_context_t *ctx, const gl_pixel_fmt_t *f,
    ...
    if (!pixels || width <= 0 || height <= 0 || depth <= 0)
        return (void *)0;
    /* A GL_BITMAP image as glBitmap's is kept: tight, most significant bit first, which
     * the neutral replay state reads back the same way. One slice. */
    if (f->bitmap) {
        return (depth == 1)
                   ? gl_bitmap_copy_client(ctx, (const GLubyte *)pixels, width, height)
                   : (void *)0;
    }
    gl_pixel_src_t s;
...
GLboolean gl_bitmap_bit(const gl_context_t *ctx, const GLubyte *bits, GLsizei width,
    ...
    const size_t bit = (size_t)ctx->unpack_skip_pixels + (size_t)x;
    const GLubyte byte =
        bits[((size_t)ctx->unpack_skip_rows + (size_t)y) * stride + bit / 8u];
    const unsigned shift =
        ctx->unpack_lsb_first ? (unsigned)(bit % 8u) : 7u - (unsigned)(bit % 8u);
 ...
 * `_mesa_base_tex_format` (main/glformats.c:2408) for a compatibility context, cut to
 * GL 1.x: the sized formats of GL 1.1, the legacy 1 to 4, GL 1.3's generic compressed
 * formats, and GL 1.4's depth formats.
 * ------------------------------------------------------------------------- */
...
/* The specification's conversion from RGBA to internal components takes luminance and
 * intensity from red (GL 1.3, table 3.15), so a GL_RGB image uploaded as GL_LUMINANCE
 * keeps its red. */
...
void gl_tex_readback_row(uint8_t *rgba, GLsizei width, GLenum base) {
    if (base != GL_LUMINANCE && base != GL_LUMINANCE_ALPHA && base != GL_INTENSITY)
        return;
    for (GLsizei i = 0; i < width; i++) {
        uint8_t *p = rgba + (size_t)i * 4u;
        p[1] = p[2] = 0u;
