 ...
 *
 * **There is no implicit conversion between int and float.** `1 + 1.0` is an error in
 * GLSL 1.10, not an int promoted to a float. Writing the usual arithmetic conversions
 * here would accept shaders the specification rejects and, worse, would silently pick a
 * type for expressions whose author meant something else.
 *
 * **A scalar against a vector is component-wise, and keeps the vector's type.** `vec3 *
 * float` is a `vec3`. So is `float * vec3`. That asymmetry-that-isn't is the common
 * case in shader code and is the first thing to get right.
 *
 * **`mat * vec` is a linear transform, not a component-wise multiply.** `mat4 * vec4`
 * is a `vec4`; `mat4 * vec3` is an error, because the dimensions do not meet. This is
 * the rule that makes `mvp * vec4(pos, 1.0)` the idiom it is, and the one a
 * component-wise implementation gets wrong while still producing a plausible type.
 */
 ...
 *
 * Wrappers rather than the statics made public, so every call site in this file keeps
 * using the short names and there is still exactly one definition of each rule. A
 * second copy of "what counts as a vector" in another file is the kind of thing that
 * agrees on the day it is written and not afterwards. */
...
/* Defined with the other array rules, below the expression checker that needs it: `==`
 * has to ask about whole arrays before it compares element types, and the array rules
 * sit beside the assignment ones rather than beside the operators. */
...
void glsl_sema_init(glsl_sema_t *s, glsl_ast_t *ast) {
    ...
    /* **Cleared for the same reason the parser's name list is**: the caller's
     * `glsl_sema_t` is often a stack local and is not zeroed, and a stale count here
     * would have `glsl_struct_of` hand back a struct made of whatever the stack held.
     */
    s->struct_count = 0;
}
 ...
 *
 * **int to float, and `ivecN` to `vecN`. That is the whole of it** (1.20, 4.1.10). Not
 * float to int, not bool to anything, and not in the other direction - so `int i
 * = 1.0;` is still an error in 1.20, which is the half people expect to work and which
 * does not.
 *
 * 1.10 converts nothing at all, and that refusal is deliberate rather than incidental:
 * without it `1 + 1.0` silently picks a type its author did not write.
 * ------------------------------------------------------------------------- */
...
/* **A matrix's shape, from a table.** `matCxR` is C columns of R rows; the square names
 * are the ones the language gives short spellings to, and `mat3` is `mat3x3`. Zero for
...
/* **The size of anything, including a struct**, which `glsl_type_components` cannot
 * answer on its own: it takes a type and a struct's size lives in the table beside it.
 * Every caller that may see a struct asks this one instead. */
...
void glsl_scope_pop(glsl_sema_t *s) {
     ...
     * tail is enough - and it has to happen here rather than being left for a later
     * lookup to filter, or a name would stay visible after its block closed. */
...
static const glsl_symbol_t *lookup(const glsl_sema_t *s, const char *name, size_t len) {
     ...
     * Forwards would find the outer one and quietly use the wrong variable. */
...
/*
 * **An integral constant expression** - GLSL 4.1.9, and the only place the language
 * needs one is an array's length.
 *
 * Folded here rather than demanded as a literal, because `const int N = 8;` followed by
 * `uniform vec2 offs[N];` is what shaders actually write - it is the shape mesa-demos'
 * `vpglsl` uses throughout, and refusing it was refusing the idiom rather than an edge
 * case.
 *
 * What folds: an integer literal, a `const` integer scalar whose own initialiser
 * folded, and the arithmetic the specification allows over those. Anything else returns
 * false and the caller refuses the declaration by name - **nothing here guesses a
 * length**, because a wrong one turns `vec4 v[n]` into something indexed out of bounds
 * at run time rather than into a diagnostic.
 *
 * Division by zero folds to false for the same reason: a constant expression the shader
 * wrote that has no value is a shader to refuse, not one to give an arbitrary length
 * to.
 */
...
GLboolean glsl_declare(glsl_sema_t *s, const char *name, size_t len, glsl_type_t type,
    ...
    /* **Redeclaration is only an error in the *same* scope.** Shadowing an outer name
     * is legal and common, so the check is against the current depth rather than the
     * whole table.
     *
     * **A function is exempt, because GLSL overloads by signature.** `permute` may be
     * declared for `float`, `vec2`, `vec3` and `vec4` - that is how the built-in
     * library is shaped and how shaders are written against it. Whether two *functions*
     * of one name are distinct is a question about their parameters, which this does
     * not have, so `glsl_declare_function` asks it. What stays an error here is a
     * function and a variable sharing a name. */
    for (int i = s->count - 1; i >= 0; i--) {
    ...
    /* **Cleared, because this table is reused and not zeroed.** A slot that held a
     * `const int` earlier would otherwise hand its value to whatever is declared here
     * next, and the symptom would be an array silently taking some previous constant's
     * length. */
 ...
 * position, colour and texture coordinates. **They may not be mixed**: `v.xg` is an
 * error, not a component named `x` and one named `g`. Mixing is exactly the typo that
 * produces a shader that compiles elsewhere and not here, or the reverse, so it is
 * worth refusing precisely.
 * ------------------------------------------------------------------------- */
...
static glsl_type_t swizzle_type(glsl_sema_t *s, glsl_type_t operand, const char *field,
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        /* **Past the end of the operand.** `.w` on a vec3 is the error this catches,
         * and it is the one that otherwise reads whatever sits after the vector. */
        if (idx >= width) {
 ...
 * is the long spelling of `matN` and denotes the same type - the language gives both
 * names. */
...
/*
 * **A GLSL 1.20 array constructor, `T[N](a, b, ...)`**, which the parser already
 * produces the right shape for without knowing it: `float[2](x, y)` is a call whose
 * *callee* is an index - `CALL(INDEX(IDENTIFIER "float", 2), args)` - because `float`
 * is a type name in primary position, `[2]` is the postfix index, and `(...)` is the
 * postfix call. Nothing had to be added to the grammar; what was missing was anyone
 * recognising the shape.
 *
 * Answers the element type and the length. It does **not** answer an array type,
 * because this type system has none: a symbol carries `(type, array_size)` and an
 * expression carries a type alone. That is why the length comes back through a pointer
 * and why an array constructor is only usable where something is waiting to receive a
 * length - a declaration's initialiser. GLSL 1.20 also allows one as an argument and a
 * return value; those stay refused, and the refusal says which, rather than pretending
 * the type system can hold an array-valued expression.
 */
GLboolean glsl_array_ctor_of(glsl_sema_t *s, int32_t node, glsl_type_t *elem,
    ...
    if (idx->b == GLSL_NO_NODE || !const_int_eval(s, idx->b, &len) || len < 1) {
        /* `float[](a, b)` - the unsized form - and `float[n]` with a length nothing can
         * fold. Both parse as far as here and neither is something to guess a length
         * for. */
        sema_fail(
            s,
    ...
    /* **Folded back into the tree**, exactly as a declarator's length is and for the
     * same reason: both back ends read this node and neither has a semantic pass to
     * ask. It is what lets `glsl_array_ctor_shape` be an AST-only question. */
...
/* Whether this looks like an array constructor whatever its length folds to - a call on
 * an index into a type name. Used only to pick the diagnostic: the unfoldable and
 * unsized forms have to be told they are array constructors in the wrong place rather
 * than "calling something that is not a name", which reads as a typo. */
...
/* The arity and element types of an array constructor, once its shape is known. 1.20
 * gives no filling rule for one - unlike `vec4(1.0)`, an array constructor takes
 * exactly one argument per element (1.20, 5.4.4). */
 ...
 *
 * GLSL's rule is by **component count, not argument count**: `vec4(v3, 1.0)` is four
 * components from two arguments and is legal, and so is `vec4(1.0)`, which fills all
 * four. Counting arguments instead would reject the first and accept `vec4(1.0, 2.0)`,
 * which is neither one component nor four. */
static glsl_type_t constructor_type(glsl_sema_t *s, glsl_type_t target,
    ...
    /* One scalar fills everything - `vec4(0.0)`, and `mat4(1.0)` which is the identity
     * scaled. */
     ...
     * components to be dropped when a single argument is larger; several arguments
     * summing past the target is a mistake. */
...
static glsl_type_t binary_type(glsl_sema_t *s, glsl_token_type_t op, glsl_type_t l,
    ...
    /* **One side float and the other int widens the int one** (GLSL 1.20). Done here,
     * once, so every rule below sees a pair that already agrees on its base type and
     * none of them has to know about the conversion. `vec3 * 2` becomes `vec3 * 2.0`;
     * `ivec2 == vec3` widens and is still a width mismatch, which is what it should be.
     */
    if (s && s->version >= 120) {
    ...
    case GLSL_TOK_NE: {
         ...
         * `glsl_type_of` answers a whole array with its *element*'s type, so `v == w`
         * on two `float[2]` arrives here as FLOAT against FLOAT and would pass the
         * equality below having compared nothing about the lengths. The array question
         * is asked first. */
    ...
    case GLSL_TOK_LE:
    case GLSL_TOK_GE:
        /* **Ordering is scalars only.** GLSL has `lessThan()` for vectors precisely
         * because
    ...
    /*
     * **`mat * vec` and `vec * mat` are linear transforms, and the two are not the same
     * one.**
     *
     * `matCxR * vecC` is `vecR` - the vector is a column, so it has one entry per
     * *column* of the matrix and the result has one per row. `vecR * matCxR` is the
     * product with the transpose: the vector is a row, so it matches the rows and the
     * result has one entry per column. For a square matrix both read as "the
     * dimension", which is why this worked while only square ones existed.
     */
    ...
    /* **`matCxR * matPxC` is `matPxR`**: the left's columns have to match the right's
     * rows, and the result takes the right's columns and the left's rows. Equal types
     * were the whole rule while every matrix was square, and that is the special case
     * where P = C = R. */
    ...
    /* **No implicit conversion.** `1 + 1.0` is an error in GLSL 1.10, and accepting it
     * would pick a type the author did not write. */
    if (l != r) {
...
glsl_type_t glsl_type_of(glsl_sema_t *s, int32_t node) {
    ...
    case GLSL_NODE_IDENTIFIER: {
        ...
        if (!sym) {
             ...
             * at.** `gl_LightSource[0]` and `gl_PointCoord` are real GLSL and are
             * missing here for reasons an author can act on - no struct type, no point
             * sprites - where "undeclared name" invites them to check their spelling
             * instead. */
    ...
    case GLSL_NODE_FIELD: {
        ...
        if (base == GLSL_TYPE_ERROR)
            return GLSL_TYPE_ERROR;
        /* **A struct member and a swizzle are the same syntax and different
         * questions.** `v.xy` asks for components of a vector; `s.a` asks for a named
         * member. The base's type decides which, and asking the wrong one produces a
         * confusing diagnostic rather than a wrong answer - `swizzle_type` would report
         * that `a` is not a component set. */
        if (glsl_type_is_struct(base)) {
            ...
            /* An array member's type is its element type, exactly as an array
             * variable's is in the symbol table - indexing is the only thing either can
             * do. */
            return m->type;
        }
    ...
    case GLSL_NODE_INDEX: {
         ...
         * `vec4 v[4]` has element type `vec4` in the table - there is no array type -
         * and typing the operand first would report a vec4 and then hand back a float
         * as though a component had been asked for. GLSL 1.10 has no array-valued
         * expressions, so an array can only ever be indexed through its own name and
         * this is the whole rule. */
        {
            ...
            /* **A member that is an array is indexed the same way**, and `s.w[0]`
             * reaches here with a field rather than a name underneath. GLSL 1.10 allows
             * an array as a struct member (4.1.9) and the member table has carried its
             * length all along
             * - both back ends lay the elements out end to end from the member's offset
             * - so the only thing missing was this rule. Without it a field's type came
             * back as the *element* type, which is not a vector or a matrix, and the
             * index was refused for indexing something that cannot be indexed. */
        ...
        /* Indexing a matrix gives a **column**, which has one entry per row - so
         * `mat2x3[0]` is a `vec3` and not a `vec2`. Indexing a vector gives a
         * component. */
        if (is_matrix(base))
    ...
    case GLSL_NODE_CALL: {
        ...
        if (callee->kind != GLSL_NODE_IDENTIFIER) {
            /* **`float[2](a, b)` is a call whose callee is an index**, and reaching
             * *here* means it is being used as a general expression - which it cannot
             * be. An array constructor's value is an array, and an expression in this
             * type system carries a type while only a symbol carries a length. So it is
             * legal in exactly one place, a declaration's initialiser, where the
             * declaration supplies the length; `check_declarator` handles it there and
             * never asks this function.
             *
             * GLSL 1.20 also allows one as an argument and as a return value. Those
             * would need an array type, and the refusal says so rather than typing the
             * constructor as its element and letting the width mismatch surface
             * somewhere else. */
         ...
         * parser refuses the declaration; this refuses `mat2x3(1.0)[0]` in an
         * expression, which never passes through a declaration and would otherwise be
         * the one way into the type in a 1.10 shader. */
         ...
         *
         * `vec4(1.0)` fills, `vec4(v3, 1.0)` gathers, `mat3(m4)` truncates - the
         * built-in constructors are a small language of their own. A struct's is none
         * of that: one argument per member, in order, each assignable to that member
         * (1.10, 5.4.3). Writing it as another case of `constructor_type` would have
         * meant teaching that function to sometimes stop doing the thing it exists to
         * do.
         */
        {
            ...
            if (st_type != GLSL_TYPE_ERROR) {
                ...
                for (int32_t a = n->b; a != GLSL_NO_NODE;
                    ...
                    /* **An array member cannot be given a value here.** GLSL 1.10 has
                     * no array-valued expression, so there is nothing that could be
                     * passed for one - a constructor for such a struct cannot be
                     * written at all, and saying so beats accepting an argument that
                     * fills only the first element. */
        ...
        /* **The built-in library, before the symbol table.** `sin`, `dot` and
         * `texture2D` are not declared anywhere - they are overloaded over genType,
         * which one signature per name cannot express - so they are resolved by rule,
         * like the constructors above. A shader may still declare a function of its own
         * with a built-in's name, which GLSL 1.10 allows; this finds the built-in
         * first, which is wrong in that one case and right in every other, and the case
         * is rare enough that the alternative - searching the table first and so paying
         * a lookup on every `sin` - is the worse trade. */
        {
            ...
            for (int32_t a = n->b; a != GLSL_NO_NODE; a = s->ast->nodes[a].sibling) {
                ...
                /* **Every integer argument widens under 1.20**, before the built-in's
                 * own rule looks at it - so `mod(x, 2)` and `clamp(v, 0, 1)` resolve,
                 * which is how shader authors write them. Nothing is lost by widening
                 * everything: neither 1.10 nor 1.20 has a built-in that takes an
                 * integer and means it, so there is no overload this could pick
                 * wrongly. */
         ...
         *
         * A name no longer identifies a function, so the arguments are evaluated once
         * and then offered to every function of that name. An exact match wins
         * outright; a match that needed a conversion `glsl_type_accepts` allows
         * - 1.20's int-to-float - is taken only when nothing matched exactly, which is
         * the rule that stops `f(int)` being passed over for `f(float)` when the shader
         * wrote an integer.
         */
        ...
        if (!sym) {
             ...
             * variable called like a function is a different mistake from an argument
             * list no overload takes. */
            if (any_non_function) {
 ...
 * L-values
 *
 * What may sit on the left of an assignment. The interesting cases are not the obvious
 * ones:
 *
 * **A swizzle with a repeated component is not assignable.** `v.xx = vec2(1.0, 2.0)`
 * would have to write two different values into one component. GLSL says so explicitly,
 * and an implementation that only checks "is this a field selection" accepts it and
 * produces whichever value happened to land last.
 *
 * **A uniform, an attribute or a const is read-only**, so its name is an l-value
 * syntactically and not semantically. That check needs the storage qualifier, which is
 * why the symbol carries one.
 * ------------------------------------------------------------------------- */
 ...
 * expression and deciding it may be written to are different questions and only
 * statements ask the second one. Walks the whole expression so a nested assignment is
 * caught too. */
/* Whether this node is a name - or a struct's member - standing for a whole array
 * rather than for one of its elements. Both spellings reach here: `a` for a declared
 * array, and `s.w` for a member declared with a length. */
/* **The element type and length of a node that names a whole array**, or false. Both
 * spellings: `a` for a declared array, `s.w` for a struct member declared with a
 * length. The type system has no array type, so this is how any rule that needs both
 * halves asks for them. */
 ...
 * `=`, `==` and `!=` take. Answers false when neither side is an array, so a caller can
 * ask first and fall through to the ordinary rules. `mismatch` distinguishes "not
 * arrays" from "arrays that do not match", because those want different diagnostics. */
static GLboolean both_whole_arrays(glsl_sema_t *s, int32_t a, int32_t b, int *count,
                                   GLboolean *mismatch) {
...
static GLboolean check_assign_targets(glsl_sema_t *s, int32_t node) {
    ...
    if (n->kind == GLSL_NODE_ASSIGN) {
        ...
        /* **A whole array is not an l-value.** GLSL 1.10 section 5.8 lists what is -
         * built-in types, entire structures, fields, swizzles without repeats, and
         * l-values in parentheses - and an array is not among them.
         *
         * Asked here rather than in `glsl_is_lvalue`, because that function recurses
         * *through* the array's name on the way to `a[0]`: refusing an array identifier
         * there would refuse every element assignment with it. The question is only
         * about a name standing alone as the whole destination, which is exactly what
         * this node's left child is.
         *
         * It was caught before this - by the *code generator*, which has no memory to
         * copy an array into. That is one back end refusing it and the interpreter,
         * which has a float array and would happily have copied something, never being
         * asked. A language error belongs to the front end so that both paths refuse it
         * for the same reason. */
        if (names_whole_array(s, n->a)) {
            /* **GLSL 1.10 5.8 lists what an l-value is and an array is not among
             * them**, so there a whole array is not assignable at all. 1.20 relaxed
             * that alongside adding array constructors, and it is implemented - the
             * rule below is the 1.20 one. */
            if (s->version < 120) {
                sema_fail(
                    s,
            ...
            /* **Only plain `=`.** A compound assignment is an arithmetic operator and
             * an assignment together, and 1.20 gives arrays neither - `a += b` would
             * have to mean an elementwise add the language does not define. */
            if (n->op != GLSL_TOK_ASSIGN) {
                sema_fail(
                    s,
 ...
 * `sibling`, and `sibling` is also how the enclosing statement or unit list is walked -
 * so whoever is walking that list reaches the second declarator on its own. Following
 * the chain here as well would declare every name twice and report a redeclaration that
 * the source does not contain. */
/* **The type a declaration or parameter node writes**, which is its token unless the
 * token is an identifier - and then it is the struct that identifier names. Every place
 * that used to call `glsl_type_from_token` on a node calls this, so a struct is
 ...
 *
 * **Layout is members end to end, in declaration order**, and the offsets computed here
 * are what both back ends use: the interpreter indexes a float array with them and the
 * code generator adds them to a register base. One layout, decided once, or the two
 * would disagree about where `s.b` is and only one of them would be wrong at a time.
 */
static GLboolean check_struct_def(glsl_sema_t *s, int32_t d) {
    ...
    for (int32_t m = n->a; m != GLSL_NO_NODE; m = s->ast->nodes[m].sibling) {
         ...
         * no layout to give it. GLSL 1.10 allows it in principle; this back end does
         * not have anywhere to put it, and says so rather than computing a size of
         * zero. */
    ...
    /* Checked once the whole thing is measured, so the message is about the struct
     * rather than about whichever member happened to cross the line. */
 ...
 * whether or not this implementation happens to have a built-in of that name. Checked
 * at the shader's own declarations rather than inside `glsl_declare`, because that is
 * what the built-in tables use to put the real ones in. */
...
static GLboolean check_declarator(glsl_sema_t *s, int32_t d) {
    ...
    if (n->a != GLSL_NO_NODE) {
        /* **An array constructor is handled here and nowhere else** - this is the one
         * context that supplies the length its value needs, so `glsl_type_of` refuses
         * it everywhere and the declarator asks about it directly. The length is
         * checked against the declared one below, once that has been folded. */
        glsl_type_t ael = GLSL_TYPE_ERROR;
        int acount = 0;
    ...
    /* **An array's length has to be an integral constant expression** - GLSL 4.1.9 -
     * which is a literal, a `const` integer, or arithmetic over those. `const_int_eval`
     * is the whole of that rule; what it cannot fold is refused by name rather than
     * assumed to be 1, which would turn `vec4 v[n]` into a scalar and index it out of
     * bounds at run time. An unsized declaration - `varying vec4 v[];` - is refused for
     * the same reason. */
    int elements = 0;
    if (n->array_size != GLSL_NO_NODE) {
        int len = 0;
         ...
         *
         * The interpreter, the generator and the linker all read an array's length
         * straight off this node and all three accepted only `GLSL_NODE_INTCONST` -
         * which is right, because a length is a number by the time anything downstream
         * wants it. Folding here and leaving `const int N = 2; float a[N];` as an
         * identifier in the tree made the semantic pass agree the length was 3 while
         * every consumer read 0: the shader compiled, linked, and drew black, and
         * `glGetUniformLocation` could not find `vals[3]` because the linker had
         * enumerated no elements.
         *
         * Replacing the expression with its value is the same discipline the struct
         * layout follows - decided once, in this pass, and read by everyone after it -
         * and it means no back end needs a constant folder of its own to stay in
     ...
     * Only an unqualified-length integer scalar with a foldable initialiser qualifies,
     * which is the set 4.1.9 allows to appear in one. */
...
static GLboolean check_statement(glsl_sema_t *s, int32_t node) {
    ...
    /* A struct declared inside a function. Its type is recorded in the unit's table
     * rather than a scoped one: GLSL 1.10 has no way to declare two structs of the same
     * name in different scopes without `check_struct_def` refusing the second, so one
     * table holds everything and the refusal is what keeps it unambiguous. */
    ...
    case GLSL_NODE_FOR: {
         ...
         * - `for (int i = 0; ...)` must not leave `i` behind, and two such loops in a
         * row must not collide. */
        glsl_scope_push(s);
        GLboolean ok = check_statement(s, n->a);
        if (ok)
    ...
    case GLSL_NODE_CONTINUE:
         ...
         * since it has no idea where it is. */
        if (s->loop_depth == 0) {
    ...
    default:
        /* An expression node reaching statement position, which the parser wraps - so
         * this is a tree built by hand rather than parsed. Typed anyway rather than
         * ignored. */
 ...
 * and has to put the unit's functions back into it: a call is typed against the
 * recorded signature, and a table that holds only the globals cannot type one at all -
 * which read as "only float, vec and mat constructor arguments are generated" the first
 * time a helper's result was passed to a constructor. */
GLboolean glsl_declare_function(glsl_sema_t *s, int32_t node) {
    ...
    /* **The signature is built before the symbol**, because whether this declaration is
     * a new overload or a redefinition is a question about the parameters, and the
     * table cannot be asked until they are known. */
     ...
     * GLSL overloads on the parameter list and *not* on the return type, so `float
     * f(int)` and `vec2 f(int)` are a redefinition rather than two overloads - a call
     * could not choose between them, since the arguments are all a call site offers.
     *
     * A prototype followed by its definition is the ordinary way to arrive here twice
     * with the same signature, so it is accepted: the second one takes over the entry,
     * which is what gives the definition's body to a call that was checked against the
     * prototype.
     */
    for (int i = s->count - 1; i >= 0; i--) {
...
GLboolean glsl_check_unit(glsl_sema_t *s, int32_t unit) {
    ...
    /* **Three passes over the top level**, and the order of the first two is
     * load-bearing.
     *
     * Structs are recorded first, because a function's signature may be written in
     * terms of one
     * - `S bump(S v)` - and the second pass resolves those types. With the struct pass
     * second, `S` was not yet a type when `bump` was declared, and a shader that passes
     * a struct to a function failed with "declaration of an unknown type" while the
     * same struct worked perfectly as a local. Found by its own test.
     *
     * Then every function is declared before any body is checked, so a function may
     * call one defined later in the file - ordinary in a shader, and otherwise an
     * undeclared-name error that depends on the order somebody happened to write them
     * in. */
    ...
    for (int32_t d = u->a; d != GLSL_NO_NODE; d = s->ast->nodes[d].sibling) {
         ...
         * collide - which is what GLSL says. */
        glsl_scope_push(s);
        ...
        for (int32_t p = n->b; p != GLSL_NO_NODE; p = s->ast->nodes[p].sibling) {
            ...
            /* **A parameter may be an array** (1.10, 6.1), and it binds exactly as a
             * local array does: the symbol carries the element type and the length, and
             * indexing is the only thing either can do. Declared without the length,
             * `w[0]` inside the body asked the index rule to index a `float` and was
             * refused for indexing something that is neither a vector nor a matrix. */
            int psize = 0;
            ...
            if (psize > 0) {
                /* **Folded back into the tree, for the same reason a declarator's
                 * length is**: the interpreter and the generator both read a length
                 * straight off this node and both want a number by then. Leaving `float
                 * w[N]` as an identifier would have the semantic pass agree the length
                 * is N while every consumer read 0. */
        ...
        /* **The body's statements are walked here, not through the COMPOUND case**,
         * because that would push a second scope and make a local merely *shadow* a
         * parameter. GLSL puts the parameters and the body's outermost block in one
         * scope, so `float f(float x){ float x; }` is a redefinition - which is what
         * glslang reports and what the comment above this loop claims. Going through
         * check_statement would have quietly accepted it. */
