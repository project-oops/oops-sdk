/*
 * oops-glut: the subset of GLUT that a GL 1.x program actually calls, over this SDK's display,
 * input and timing. See include/GL/glut.h for what is here and what is not.
 *
 * # The main loop
 *
 * GLUT's contract is that `glutMainLoop` never returns and the program lives in its callbacks.
 * On a console that leaves no way to stop, so `glutLeaveMainLoop` - freeglut's spelling, because
 * GLUT has none - ends it, and `glutMainLoop` returns to whatever called it.
 *
 * One pass of the loop: drain the input devices and dispatch their callbacks, fire any timer
 * that is due, then either redisplay or idle. A program that never calls `glutPostRedisplay`
 * and has no idle callback still polls input, so it can always be left.
 *
 * # Input
 *
 * The keyboard arrives as HID usage codes, which this maps to the ASCII `glutKeyboardFunc`
 * expects and the `GLUT_KEY_*` values `glutSpecialFunc` expects. Only the keys those two
 * callbacks can express are mapped; anything else is dropped rather than delivered as a
 * plausible wrong character.
 *
 * The mouse is relative, and GLUT's callbacks are absolute, so a cursor position is kept here
 * and clamped to the window. It starts at the centre, which is where a program that reads the
 * first motion event expects to find it.
 *
...
void glutInit(int *argcp, char **argv) {
    (void)argcp;
    (void)argv;
    /* GLUT strips its own arguments here; this has none to strip, and a program that passes
...
void glutInitDisplayMode(unsigned int mode) { g.mode = mode; }

void glutInitWindowSize(int width, int height) {
    if (width > 0) g.want_width = width;
    if (height > 0) g.want_height = height;
}
...
int glutCreateWindow(const char *title) {
    ...
    g.created = 1;
    /* The devices a GLUT program expects to exist. A console with none attached simply delivers
     * no events; the pad below is usually what is there. */
    oops_keyboard_init();
    oops_mouse_init();
    oops_input_init();
...
void glutMouseFunc(void (*func)(int, int, int, int)) { g.mouse = func; }
void glutMotionFunc(void (*func)(int, int)) { g.motion = func; }
void glutPassiveMotionFunc(void (*func)(int, int)) { g.passive = func; }
...
void glutVisibilityFunc(void (*func)(int)) { g.visibility = func; }
void glutOopsPadKeys(int on) { g.pad_keys = on ? 1 : 0; }

void glutTimerFunc(unsigned int millis, void (*func)(int value), int value) {
    if (!func) return;
    for (int i = 0; i < GLUT_MAX_TIMERS; i++) {
        if (g.timers[i].used) continue;
        g.timers[i].used = 1;
    ...
    /* Out of slots: GLUT has no error for this, and dropping the timer silently would stop a
     * program that reschedules from its own callback. The oldest due one is replaced. */
    int oldest = 0;
    for (int i = 1; i < GLUT_MAX_TIMERS; i++) {
        if (g.timers[i].due_ns < g.timers[oldest].due_ns) oldest = i;
    }
...
int glutGet(GLenum state) {
    switch (state) {
        ...
        case GLUT_WINDOW_WIDTH:  return g.width;
        case GLUT_WINDOW_HEIGHT: return g.height;
        case GLUT_SCREEN_WIDTH:  return g.width;
        case GLUT_SCREEN_HEIGHT: return g.height;
        case GLUT_WINDOW_RGBA:   return 1;
        case GLUT_WINDOW_DOUBLEBUFFER: return 1;
        case GLUT_WINDOW_DEPTH_SIZE:   return 32;
        ...
        case GLUT_WINDOW_ACCUM_ALPHA_SIZE: return 16;
        /* One top-level window, no children, no colour map, no multisampling, no stereo - each
         * of these is the state rather than a value this cannot produce. */
        ...
        case GLUT_WINDOW_CURSOR:       return GLUT_CURSOR_NONE;
        /* What glutInitWindowSize and glutInitDisplayMode were given, which is what GLUT reports
         * here - the request, not what was opened. */
        case GLUT_INIT_WINDOW_X:      return 0;
        case GLUT_INIT_WINDOW_Y:      return 0;
        case GLUT_INIT_WINDOW_WIDTH:  return g.want_width;
        case GLUT_INIT_WINDOW_HEIGHT: return g.want_height;
        case GLUT_INIT_DISPLAY_MODE:  return (int)g.mode;
        /* Every mode but colour-index is available; that one this library does not have, so a
         * program that asks is told before it draws nothing. */
        case GLUT_DISPLAY_MODE_POSSIBLE: return (g.mode & GLUT_INDEX) ? 0 : 1;
 ...
 * The one window
 *
 * Each of these is exact here rather than approximated - see glut.h for which of GLUT's window
 * calls are deliberately absent instead.
 * --------------------------------------------------------------------------- */

int glutGetWindow(void) { return g.created ? 1 : 0; }

/* GLUT sets the current window; there is one, so naming it is a no-op and naming anything else
 * is the error GLUT makes it - ignored, with the current window unchanged. */
void glutSetWindow(int window) { (void)window; }

/* A title has nowhere to go on this display. Accepted and ignored, as glutInitWindowPosition
 * is: a program that sets one is not asking for anything it can see fail. */
void glutSetWindowTitle(const char *title) { (void)title; }
void glutSetIconTitle(const char *title) { (void)title; }

/* The window already is the display, full screen and at its origin, so this asks for the state
 * it is permanently in. */
void glutFullScreen(void) { }

/* There is no cursor to set. glutGet(GLUT_WINDOW_CURSOR) answers GLUT_CURSOR_NONE whatever is
 * asked for, so a program that checks sees what it got. */
...
/* A HID usage code as the character GLUT's keyboard callback takes, or 0 for a key that callback
 * cannot express. Shift is applied to the letters and digits only - the punctuation rows differ
 * by layout, and a wrong character is worse than none. */
static unsigned char glut_usage_ascii(uint16_t usage, uint8_t mods) {
    ...
    switch (usage) {
        case 0x27u: return shift ? 0u : (unsigned char)'0';
        case 0x28u: return (unsigned char)'\r';   /* Return */
        case 0x29u: return (unsigned char)'\033'; /* Escape */
        case 0x2au: return (unsigned char)'\b';   /* Backspace */
        case 0x2bu: return (unsigned char)'\t';
        case 0x2cu: return (unsigned char)' ';
        case 0x2du: return shift ? 0u : (unsigned char)'-';
        case 0x2eu: return shift ? 0u : (unsigned char)'=';
        case 0x36u: return shift ? 0u : (unsigned char)',';
        case 0x37u: return shift ? 0u : (unsigned char)'.';
        case 0x38u: return shift ? 0u : (unsigned char)'/';
        default:    return 0u;
    }
}
...
static int glut_usage_special(uint16_t usage) {
    if (usage >= 0x3au && usage <= 0x45u) return GLUT_KEY_F1 + (int)(usage - 0x3au); /* F1..F12 */
    switch (usage) {
        case 0x4fu: return GLUT_KEY_RIGHT;
        case 0x50u: return GLUT_KEY_LEFT;
        case 0x51u: return GLUT_KEY_DOWN;
        case 0x52u: return GLUT_KEY_UP;
        case 0x4bu: return GLUT_KEY_PAGE_UP;
        case 0x4eu: return GLUT_KEY_PAGE_DOWN;
        case 0x4au: return GLUT_KEY_HOME;
        case 0x4du: return GLUT_KEY_END;
        case 0x49u: return GLUT_KEY_INSERT;
        default:    return 0;
    }
}
...
static void glut_pump_keyboard(void) {
    ...
    for (int i = 0; i < n; i++) {
        g.modifiers = 0;
        if (ev[i].modifiers & OOPS_KMOD_SHIFT) g.modifiers |= GLUT_ACTIVE_SHIFT;
        if (ev[i].modifiers & OOPS_KMOD_CTRL) g.modifiers |= GLUT_ACTIVE_CTRL;
        if (ev[i].modifiers & OOPS_KMOD_ALT) g.modifiers |= GLUT_ACTIVE_ALT;
        const int down = ev[i].transition == OOPS_KEY_DOWN;
        const int special = glut_usage_special(ev[i].usage);
        if (special) {
            if (down && g.special) g.special(special, g.mouse_x, g.mouse_y);
            if (!down && g.special_up) g.special_up(special, g.mouse_x, g.mouse_y);
            continue;
        }
        const unsigned char ch = glut_usage_ascii(ev[i].usage, ev[i].modifiers);
        if (!ch) continue;
        if (down && g.keyboard) g.keyboard(ch, g.mouse_x, g.mouse_y);
        if (!down && g.keyboard_up) g.keyboard_up(ch, g.mouse_x, g.mouse_y);
    }
}
...
static void glut_pump_mouse(void) {
    ...
    for (int i = 0; i < n; i++) {
        if (s[i].dx || s[i].dy) {
            ...
            if (g.mouse_x >= g.width) g.mouse_x = g.width - 1;
            if (g.mouse_y >= g.height) g.mouse_y = g.height - 1;
            /* GLUT's rule: motion while a button is down, passive motion otherwise. */
            if (g.mouse_buttons && g.motion) g.motion(g.mouse_x, g.mouse_y);
            else if (!g.mouse_buttons && g.passive) g.passive(g.mouse_x, g.mouse_y);
        }
        const uint8_t changed = (uint8_t)(s[i].buttons ^ g.mouse_buttons);
        if (changed && g.mouse) {
            static const struct { uint8_t bit; int button; } map[3] = {
                {OOPS_MOUSE_LEFT, GLUT_LEFT_BUTTON},
                {OOPS_MOUSE_MIDDLE, GLUT_MIDDLE_BUTTON},
                {OOPS_MOUSE_RIGHT, GLUT_RIGHT_BUTTON},
            };
            for (int b = 0; b < 3; b++) {
                if (!(changed & map[b].bit)) continue;
                g.mouse(map[b].button, (s[i].buttons & map[b].bit) ? GLUT_DOWN : GLUT_UP,
                        g.mouse_x, g.mouse_y);
            }
        }
...
/*
 * The pad as keys. A GLUT program has no notion of a controller, and a console often has no
 * keyboard, so without this many ports run and cannot be controlled. Edges only - a held button
 * is one event, as a key press is - and the option button ends the loop, which is the way out of
 * a program whose own is a window close.
 */
static void glut_pump_pad(void) {
    if (!g.pad_keys) return;
    oops_pad_state_t pad;
    if (oops_input_poll(0u, &pad) != 0) return;
    const uint32_t now = pad.buttons, was = g.pad_buttons;
    const uint32_t pressed = now & ~was, released = was & ~now;
    g.pad_buttons = now;
    static const struct { uint32_t bit; int special; unsigned char ch; } map[] = {
    ...
    for (size_t i = 0; i < sizeof(map) / sizeof(map[0]); i++) {
        if (pressed & map[i].bit) {
            if (map[i].special && g.special) g.special(map[i].special, g.mouse_x, g.mouse_y);
            else if (map[i].ch && g.keyboard) g.keyboard(map[i].ch, g.mouse_x, g.mouse_y);
        }
        if (released & map[i].bit) {
            if (map[i].special && g.special_up) g.special_up(map[i].special, g.mouse_x, g.mouse_y);
            else if (map[i].ch && g.keyboard_up) g.keyboard_up(map[i].ch, g.mouse_x, g.mouse_y);
        }
    }
    if (pressed & OOPS_BUTTON_OPTIONS) glutLeaveMainLoop();
}

static void glut_fire_timers(void) {
    const uint64_t now = oops_time_get_ns();
    for (int i = 0; i < GLUT_MAX_TIMERS; i++) {
        if (!g.timers[i].used || g.timers[i].due_ns > now) continue;
        void (*f)(int) = g.timers[i].func;
        const int v = g.timers[i].value;
        /* Cleared before the call, so a callback that reschedules itself gets this slot. */
        g.timers[i].used = 0;
...
void glutLeaveMainLoop(void) { g.running = 0; }

void glutMainLoop(void) {
    if (!g.created && !glutCreateWindow("")) return;
    g.running = 1;
    /* GLUT delivers one reshape before the first display, which is where a program sets its
     * projection, and one visibility event for a window that has just come up. */
    if (g.reshape) g.reshape(g.width, g.height);
...
static void glut_sphere_body(GLUquadric *q) { gluSphere(q, s_radius, s_slices, s_stacks); }
static void glut_cone_body(GLUquadric *q) {
    gluCylinder(q, s_base, 0.0, s_height, s_slices, s_stacks);
}

void glutSolidSphere(GLdouble radius, GLint slices, GLint stacks) {
    s_radius = radius; s_slices = slices; s_stacks = stacks;
...
void glutSolidCone(GLdouble base, GLdouble height, GLint slices, GLint stacks) {
    s_base = base; s_height = height; s_slices = slices; s_stacks = stacks;
...
/* The cube, centred on the origin with sides of `size`, one quad a face with its outward normal.
 * GLUT's is the same six faces; nothing here is sampled from it. */
static void glut_cube(GLdouble size, GLenum mode) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        for (int k = 0; k < 4; k++) glVertex3f(v[f[i][k]][0], v[f[i][k]][1], v[f[i][k]][2]);
        glEnd();
    }
}

void glutSolidCube(GLdouble size) { glut_cube(size, GLU_FILL); }
void glutWireCube(GLdouble size) { glut_cube(size, GLU_LINE); }

/*
 * **The Platonic solids, derived rather than transcribed.**
 *
 * Every GLUT ships these as literal tables of vertices and face indices. Copying one would be
 * taking another implementation's data, and writing a table out by hand is the kind of work that
 * is wrong in one entry and looks right everywhere - a dodecahedron with one face wound backwards
 * draws almost correctly under the default cull. So the vertices come out of the definitions and
 * the faces are found from the vertices.
 *
 * The vertices:
 *   - tetrahedron: four alternate corners of the cube (+-1, +-1, +-1) - those with an even number
 *     of minus signs. Radius sqrt(3).
 *   - octahedron: the six unit axes. Radius 1.
 *   - icosahedron: the twelve corners of three mutually perpendicular golden rectangles,
 *     (0, +-1, +-phi) and its two cyclic rotations, scaled to radius 1.
 *   - dodecahedron: the cube's eight corners plus the twelve points (0, +-1/phi, +-phi) and its
 *     rotations, all of which are at radius sqrt(3) as the cube's corners are.
 *
 * The faces: for the three triangular solids, a face is any three vertices that are pairwise the
 * shortest distance apart in the solid - which is what "edge" means. For the dodecahedron, whose
 * faces are pentagons, the twelve face normals are the icosahedron's twelve vertices, because
 * the two solids are duals; each face is the five vertices furthest along its normal, put in
 * order around it.
 *
 * The radii are GLUT's documented ones - 1 for the octahedron and the icosahedron, sqrt(3) for
 * the other two - so a program that scales expecting GLUT's sizes gets them.
 */
...
/* Every three vertices that are pairwise an edge apart, wound counter-clockwise seen from
 * outside - which for a solid centred on the origin means the face's own outward direction. */
static void glut_poly_triangles(glut_poly_t *p) {
    float emin = 0.0f;
    for (int i = 0; i < p->nv; i++) {
        for (int j = i + 1; j < p->nv; j++) {
            const float d = glut_dist2(p->v[i], p->v[j]);
            if (emin == 0.0f || d < emin) emin = d;
        }
    }
    ...
    for (int i = 0; i < p->nv; i++) {
        for (int j = i + 1; j < p->nv; j++) {
            if (glut_dist2(p->v[i], p->v[j]) > emin + tol) continue;
            for (int k = j + 1; k < p->nv; k++) {
                if (glut_dist2(p->v[i], p->v[k]) > emin + tol) continue;
                if (glut_dist2(p->v[j], p->v[k]) > emin + tol) continue;
                if (p->nf >= GLUT_POLY_MAX_F) return;
                float e1[3], e2[3], n[3];
                for (int c = 0; c < 3; c++) {
...
/* The dodecahedron's twelve pentagons, one per direction in `normals`: the five vertices
 * furthest along it, ordered around it.
 *
 * **Ordered without an arc tangent.** The key below runs from 0 to 4 as the angle runs from 0 to
 * 2pi - the first half from the dot product, the second from its reflection - which is monotone
 * in the angle and enough to sort by. */
static void glut_poly_pentagons(glut_poly_t *p, const float (*normals)[3], int count) {
    ...
    for (int fi = 0; fi < count && p->nf < GLUT_POLY_MAX_F; fi++) {
        ...
        /* An in-plane frame: u towards the first vertex, w perpendicular to it around n. */
        float u[3], w[3];
        const float c0 = glut_dot(p->v[idx[0]], n);
        for (int c = 0; c < 3; c++) u[c] = p->v[idx[0]][c] - c0 * n[c];
        glut_normalize(u);
        glut_cross(n, u, w);
        ...
        for (int s = 0; s < 5; s++) {
            ...
            for (int c = 0; c < 3; c++) r[c] = p->v[idx[s]][c] - cs * n[c];
            glut_normalize(r);
            const float t = glut_dot(r, u), sgn = glut_dot(r, w);
            key[s] = (sgn >= 0.0f) ? (1.0f - t) : (3.0f + t);
        }
        for (int a = 1; a < 5; a++) { /* five values: an insertion sort is the whole of it */
            const float kv = key[a];
            const int iv = idx[a];
            int b = a - 1;
            while (b >= 0 && key[b] > kv) { key[b + 1] = key[b]; idx[b + 1] = idx[b]; b--; }
            key[b + 1] = kv;
            idx[b + 1] = iv;
        }
        for (int s = 0; s < 5; s++) p->f[p->nf][s] = idx[s];
        p->nf++;
    }
}

/*
 * Twelve directions at radius 1, the corners of three perpendicular golden rectangles - an
 * icosahedron.
 *
 * **There are two of them and they are not the same one.** Putting the long side of each
 * rectangle after the short gives (0, +-1, +-phi) and its rotations; putting it first gives
 * (0, +-phi, +-1). Both are regular icosahedra, mirror images of each other, and only the second
 * is the dual of the dodecahedron as its vertices are written below - so only the second is the
 * set of that dodecahedron's face normals. Using the first put five vertices from three
 * different faces into each pentagon, and `test_gl_platonic_solids_are_the_solids_they_claim`
 * caught it on the planarity check: a set of five furthest along a direction that does not lie
 * in one plane is not a face.
 */
    ...
    for (int axis = 0; axis < 3; axis++) {
        for (int sa = -1; sa <= 1; sa += 2) {
            for (int sb = -1; sb <= 1; sb += 2) {
                ...
                glut_normalize(v);
                out[n][0] = v[0]; out[n][1] = v[1]; out[n][2] = v[2];
                n++;
            }
        }
    }
...
static void glut_build_poly(glut_poly_t *p, int which) {
    ...
    p->nv = 0;
    if (which == 0) { /* tetrahedron: the cube corners with an even number of minus signs */
        for (int i = 0; i < 8; i++) {
            const float x = (i & 1) ? 1.0f : -1.0f;
            const float y = (i & 2) ? 1.0f : -1.0f;
            const float z = (i & 4) ? 1.0f : -1.0f;
            if (x * y * z < 0.0f) continue;
            p->v[p->nv][0] = x; p->v[p->nv][1] = y; p->v[p->nv][2] = z;
            p->nv++;
        }
        ...
        for (int i = 0; i < 12; i++) {
            p->v[i][0] = pts[i][0]; p->v[i][1] = pts[i][1]; p->v[i][2] = pts[i][2];
        }
        p->nv = 12;
        glut_poly_triangles(p);
    } else { /* dodecahedron: the cube's corners and three more rectangles, radius sqrt(3) */
        for (int i = 0; i < 8; i++) {
...
/* The normal is the face's own centre direction, which for a solid centred on the origin is its
 * outward normal exactly - the same value for every vertex of the face, as a flat face wants. */
static void glut_draw_poly(int which, GLenum mode) {
    ...
    for (int i = 0; i < p.nf; i++) {
        ...
        for (int s = 0; s < p.fv; s++) {
            for (int c = 0; c < 3; c++) n[c] += p.v[p.f[i][s]][c];
        }
        glut_normalize(n);
        glBegin(mode == GLU_FILL ? (p.fv == 3 ? GL_TRIANGLES : GL_POLYGON) : GL_LINE_LOOP);
...
void glutSolidTetrahedron(void) { glut_draw_poly(0, GLU_FILL); }
void glutWireTetrahedron(void) { glut_draw_poly(0, GLU_LINE); }
void glutSolidOctahedron(void) { glut_draw_poly(1, GLU_FILL); }
void glutWireOctahedron(void) { glut_draw_poly(1, GLU_LINE); }
void glutSolidIcosahedron(void) { glut_draw_poly(2, GLU_FILL); }
void glutWireIcosahedron(void) { glut_draw_poly(2, GLU_LINE); }
void glutSolidDodecahedron(void) { glut_draw_poly(3, GLU_FILL); }
void glutWireDodecahedron(void) { glut_draw_poly(3, GLU_LINE); }

/* The torus: `inner` is the tube's radius and `outer` the distance from the origin to the tube's
 * centre, which is GLUT's meaning of the two and not the other reading. */
static void glut_torus(GLdouble inner, GLdouble outer, GLint sides, GLint rings, GLenum mode) {
    if (sides < 3) sides = 3;
    if (rings < 3) rings = 3;
