...
/* Scratch memory for a pixel operation that has to hold a rectangle - glCopyPixels'
 * source. */
...
static void gl_zoomed_fragments(gl_context_t *ctx, const gl_pixel_frags_t *pf, int sx,
                                int sy, const float c[4]) {
    int x0, x1, y0, y1;
    gl_zoom_span(ctx->raster_pos[0], ctx->pixel_zoom_x, sx, &x0, &x1);
    gl_zoom_span(ctx->raster_pos[1], ctx->pixel_zoom_y, sy, &y0, &y1);
    for (int y = y0; y <= y1; y++) {
        for (int x = x0; x <= x1; x++)
            gl_pixel_fragment(ctx, pf, x, y, c);
    }
}

/* One stencil index of a rectangle, into the stencil buffer across its zoomed block. A
 * stencil rectangle is not a set of fragments: its values are written directly, through
 * the scissor and GL_STENCIL_WRITEMASK and nothing else (GL 1.x, 4.3.1; Mesa swrast's
 * draw_stencil_pixels writes the span through the mask). The stencil buffer is the
 * CPU's on both paths. */
static void gl_zoomed_stencil(gl_context_t *ctx, const gl_pixel_frags_t *pf, int sx,
                              int sy, int64_t s) {
    ...
    for (int y = y0; y <= y1; y++) {
        if (y < pf->sc_y0 || y > pf->sc_y1)
            continue;
        for (int x = x0; x <= x1; x++) {
            if (x < pf->sc_x0 || x > pf->sc_x1)
                continue;
...
/* The raster colour as a fragment's: what a depth rectangle's fragments are coloured,
 * clamped as glBitmap's are - the raster position keeps it unclamped when unlit. */
...
void glRasterPos4f(GLfloat x, GLfloat y, GLfloat z, GLfloat w) {
    ...
    rv.z = z;
    rv.w = w;
    rv.nx = ctx->cur_normal[0];
    rv.ny = ctx->cur_normal[1];
    rv.nz = ctx->cur_normal[2];
     ...
     * GL 1.4's GL_FOG_COORD source it is the current fog coordinate instead (Mesa
     * main/rastpos.c:475-479). */
    float eye[4];
    mat4_transform_vec4(eye, &ctx->modelview_stack[ctx->modelview_depth], obj);
    ctx->raster_distance =
        (ctx->fog_coord_src == GL_FOG_COORD)
            ? ctx->cur_fog_coord
            : gl_sqrt(eye[0] * eye[0] + eye[1] * eye[1] + eye[2] * eye[2]);
    ctx->raster_valid = GL_TRUE;
    /* In GL_SELECT a valid raster position is a hit, like a point (Mesa
     * main/rastpos.c:523). */
 ...
 * distance 0 (without fog coordinates). A valid raster position is a hit in GL_SELECT,
 * as glRasterPos's is. */
void glWindowPos3f(GLfloat x, GLfloat y, GLfloat z) {
    if (gl_list_recording() &&
        GL_LIST_REC(GL_LIST_OP_WINDOW_POS, gl_la_f(x), gl_la_f(y), gl_la_f(z)))
        return;
    gl_context_t *ctx = gl_get_ctx();
    if (!ctx)
        return;
    ...
    ctx->raster_distance =
        (ctx->fog_coord_src == GL_FOG_COORD) ? ctx->cur_fog_coord : 0.0f;
    for (int i = 0; i < 4; i++) {
        const float c = ctx->cur_color[i];
        ctx->raster_color[i] = (c < 0.0f) ? 0.0f : ((c > 1.0f) ? 1.0f : c);
    }
    memcpy(ctx->raster_texcoord, ctx->cur_texcoord,
           sizeof(ctx->raster_texcoord)); /* every unit's */
    gl_fb_raster_hit(ctx);
}
...
void glPixelZoom(GLfloat xfactor, GLfloat yfactor) {
    if (gl_list_recording() &&
        GL_LIST_REC(GL_LIST_OP_PIXEL_ZOOM, gl_la_f(xfactor), gl_la_f(yfactor)))
        return;
    gl_context_t *ctx = gl_get_ctx();
    if (!ctx)
        return;
...
void glDrawPixels(GLsizei width, GLsizei height, GLenum format, GLenum type,
    ...
    /* Selection and feedback draw nothing; feedback reports the rectangle's position,
     * whatever its size (Mesa main/drawpix.c:164-171). */
    if (gl_fb_active(ctx)) {
    ...
    for (int sy = 0; sy < height; sy++) {
        ...
        for (int sx = 0; sx < width; sx++) {
            ...
            if (f.kind == GL_COLOR_INDEX) {
                ...
                /* After expansion to RGBA, so an image with no alpha has its 1.0 scaled
                 * and biased like any other - the specification's order, and Mesa's.
                 * Then clamped, as a fragment's colour is for a fixed-point colour
                 * buffer. */
                if (transfer)
...
void glCopyPixels(GLint x, GLint y, GLsizei width, GLsizei height, GLenum type) {
    if (gl_list_recording() &&
        GL_LIST_REC(GL_LIST_OP_COPY_PIXELS, gl_la_i(x), gl_la_i(y), gl_la_i(width),
                    gl_la_i(height), gl_la_e(type))) {
        return;
    }
    gl_context_t *ctx = gl_get_ctx();
    if (!ctx)
    ...
    if ((type == GL_DEPTH && !ctx->depth_buffer) ||
        (type == GL_STENCIL && !ctx->stencil_buffer)) {
        gl_record_error(ctx, GL_INVALID_OPERATION); /* no such buffer */
        return;
    }
    if (!ctx->raster_valid || width == 0 || height == 0)
        return;
    if (gl_fb_active(ctx)) {
    ...
    if (type != GL_COLOR) {
        float *vals =
            (float *)gl_raster_scratch((size_t)width * (size_t)height * sizeof(float));
        if (!vals) {
        ...
        for (int sy = 0; sy < height; sy++) {
            ...
            for (int sx = 0; sx < width; sx++) {
                const int scol = x + sx;
                const GLboolean in =
                    (GLboolean)(srow >= 0 && (unsigned int)srow < ctx->height &&
                                scol >= 0 && (unsigned int)scol < ctx->width);
                vals[(size_t)sy * (size_t)width + (size_t)sx] =
                    !in ? 0.0f
                        : (type == GL_DEPTH
                               ? *gl_zs_depth_ptr(ctx, scol, y + sy)
        ...
        for (int sy = 0; sy < height; sy++) {
            const int srow = gl_raster_row(ctx, y + sy);
            if (srow < 0 || (unsigned int)srow >= ctx->height)
                continue;
            for (int sx = 0; sx < width; sx++) {
                const int scol = x + sx;
                if (scol < 0 || (unsigned int)scol >= ctx->width)
                    continue;
                float v = vals[(size_t)sy * (size_t)width + (size_t)sx];
                if (type == GL_DEPTH) {
                    if (xfer)
                        v = v * ctx->depth_scale + ctx->depth_bias;
                    vf.z = !(v > 0.0f) ? 0.0f : ((v > 1.0f) ? 1.0f : v);
                    gl_zoomed_fragments(ctx, &vf, sx, sy, colour);
                } else {
                    const int64_t st =
                        xfer ? gl_stencil_transfer(ctx, (int64_t)v) : (int64_t)v;
    ...
    for (int sy = 0; sy < height; sy++) {
        ...
        for (int sx = 0; sx < width; sx++) {
             ...
             */
            if (transfer) {
                uint8_t c8[4] = {(uint8_t)(v >> 16), (uint8_t)(v >> 8), (uint8_t)v,
                                 (uint8_t)(v >> 24)};
                gl_pixel_transfer_rgba8(ctx, c8);
                v = ((uint32_t)c8[3] << 24) | ((uint32_t)c8[0] << 16) |
                    ((uint32_t)c8[1] << 8) | c8[2];
            }
            const float c[4] = {
                (float)((v >> 16) & 0xffu) / 255.0f, (float)((v >> 8) & 0xffu) / 255.0f,
...
void glBitmap(GLsizei width, GLsizei height, GLfloat xorig, GLfloat yorig,
    ...
    if (gl_fb_active(ctx)) {
    ...
    } else if (bitmap && width > 0 && height > 0 && ctx->raster_valid &&
               ctx->framebuffer) {
        gl_raster_sync(ctx);
        ...
        /* One bit per pixel, addressed through the unpack state - row length,
         * alignment, skips and GL_UNPACK_LSB_FIRST (gl_bitmap_bit) - at floor(raster -
         * origin), GL 1.x 3.6.6. Unzoomed: glPixelZoom does not apply to bitmaps. */
...
static int16_t gl_accum_snorm(float v) {
    ...
    if (!(s > -32767.0f))
        return (int16_t)-32767; /* also NaN */
    if (s >= 32767.0f)
        return (int16_t)32767;
    return (int16_t)(s < 0.0f ? s - 0.5f : s + 0.5f);
}

/* The rectangle every operation covers, in window coordinates: the whole buffer, or the
 * scissor box within it when the scissor test is on (Mesa bounds them by the draw
 * buffer's _Xmin.._Ymax). */
static GLboolean gl_accum_region(const gl_context_t *ctx, int *x0, int *y0, int *x1,
                                 int *y1) {
    *x0 = 0;
    *y0 = 0;
    *x1 = (int)ctx->width;
...
void gl_accum_clear(gl_context_t *ctx) {
    int x0, y0, x1, y1;
    if (!gl_accum_region(ctx, &x0, &y0, &x1, &y1) || !gl_accum_ensure(ctx))
        return;
    int16_t c[4];
    for (int i = 0; i < 4; i++)
        c[i] = gl_accum_snorm(ctx->accum_clear[i]);
    for (int y = y0; y < y1; y++) {
        int16_t *row = ctx->accum_buffer + ((size_t)y * ctx->width) * 4u;
        for (int x = x0; x < x1; x++) {
            for (int i = 0; i < 4; i++)
                row[(size_t)x * 4u + (size_t)i] = c[i];
        }
    }
}
...
void glAccum(GLenum op, GLfloat value) {
    if (gl_list_recording() &&
        GL_LIST_REC(GL_LIST_OP_ACCUM, gl_la_e(op), gl_la_f(value)))
        return;
    gl_context_t *ctx = gl_get_ctx();
    if (!ctx)
        return;
    if (op != GL_ACCUM && op != GL_LOAD && op != GL_RETURN && op != GL_MULT &&
        op != GL_ADD) {
        gl_record_error(ctx, GL_INVALID_ENUM);
        return;
    }
    ...
    if (op == GL_RETURN && ctx->render_mode != GL_RENDER)
        return;
    int x0, y0, x1, y1;
    if (!gl_accum_region(ctx, &x0, &y0, &x1, &y1) || !gl_accum_ensure(ctx))
        return;
    ...
    if (op == GL_ACCUM || op == GL_LOAD) {
        /* Everything drawn so far has to have landed before it is read - glReadPixels'
         * rule. Both operations read the buffer glReadBuffer names (GL 1.0, 4.2.4). */
        glFlush();
        ...
        for (int y = y0; y < y1; y++) {
            int16_t *row = ctx->accum_buffer + ((size_t)y * w) * 4u;
            for (int x = x0; x < x1; x++) {
                /* The CP's copy is filled by a DMA the CPU has no idea about, so the
                 * line is dropped before it is read - the same rule glReadPixels
                 * follows. */
                ...
                for (int i = 0; i < 4; i++) {
                    const float prior =
                        (op == GL_ACCUM) ? (float)acc[i] / 32767.0f : 0.0f;
    ...
    /* GL_RETURN: the buffer times `value`, clamped, into the colour buffer through the
     * colour mask - read-modify-write where a channel is masked, as Mesa does
     * (accum_return) - and into each buffer glDrawBuffer names. */
    if (!ctx->framebuffer)
        return;
    gl_raster_sync(ctx);
    for (int b = 0; b < 2; b++) {
        ...
        for (int y = y0; y < y1; y++) {
            ...
            for (int x = x0; x < x1; x++) {
                ...
                const uint32_t prior[4] = {(old >> 16) & 0xffu, (old >> 8) & 0xffu,
                                           old & 0xffu, (old >> 24) & 0xffu};
                uint32_t out[4];
                for (int i = 0; i < 4; i++) {
                    if (!gl_color_writes(ctx, i)) {
                        out[i] = prior[i];
                        continue;
                    }
...
/* The polygon stipple: a 32x32 bitmap, unpacked as glBitmap unpacks one - rows of four
 * bytes, most significant bit leftmost, padded to GL_UNPACK_ALIGNMENT, the first row
 * the bottom - and kept a row to a word, byte 0 in the top bits, as Mesa keeps it
 * (main/pack.c, _mesa_unpack_polygon_stipple). Compiled with the mask packed at compile
 * time. A null mask changes nothing. */
void glPolygonStipple(const GLubyte *mask) {
    ...
    if (gl_list_recording()) {
        ...
        if (mask && !bits) {
            if (ctx->list_mode == GL_COMPILE)
                return; /* out of memory, already recorded */
        } else if (gl_list_rec_owned(GL_LIST_OP_POLYGON_STIPPLE,
                                     (const gl_list_arg_t *)0, 0, bits,
                                     bits ? (32u / 8u) * 32u : 0u)) {
            return;
        }
    }
    if (!mask)
        return;
    /* Bit by bit through the unpack state, so the skips and GL_UNPACK_LSB_FIRST apply
     * as they do to glBitmap. */
    for (int r = 0; r < 32; r++) {
        uint32_t row = 0;
        for (int c = 0; c < 32; c++) {
            if (gl_bitmap_bit(ctx, mask, 32, c, r))
                row |= 0x80000000u >> c;
        }
...
void glGetPolygonStipple(GLubyte *mask) {
    ...
    if (!ctx || !mask)
        return;
    /* Written as the pack state lays a bitmap out: row length (in bits), alignment,
     * skips and GL_PACK_LSB_FIRST. Only the mask's own bits are touched. */
    ...
    for (int r = 0; r < 32; r++) {
        ...
        for (int c = 0; c < 32; c++) {
            ...
            const unsigned shift =
                ctx->pack_lsb_first ? (unsigned)(bit % 8u) : 7u - (unsigned)(bit % 8u);
            if ((ctx->polygon_stipple[r] >> (31 - c)) & 1u)
                p[bit / 8u] |= (GLubyte)(1u << shift);
            else
                p[bit / 8u] &= (GLubyte) ~(1u << shift);
        }
    }
 ...
 *
 * Every colour a pixel rectangle carries in - glDrawPixels, glCopyPixels, the texture
 * uploads and copies - or out through glReadPixels is scaled and biased per component,
 * then with GL_MAP_COLOR replaced from its component's table, then clamped: Mesa's
 * order (main/pixeltransfer.c, _mesa_apply_rgba_transfer_ops). The table entry is the
 * component times (size - 1), rounded to the nearest with ties to even as Mesa rounds
...
GLboolean gl_pixel_transfer_active(const gl_context_t *ctx) {
    if (!ctx || ctx->pixel_transfer_suspend)
        return GL_FALSE;
    if (ctx->map_color)
        return GL_TRUE;
    for (int i = 0; i < 4; i++) {
        if (ctx->pixel_scale[i] != 1.0f || ctx->pixel_bias[i] != 0.0f)
            return GL_TRUE;
    }
    return GL_FALSE;
}
...
static int gl_round_half_even(float x) {
    /* Past what an int holds the value is its own rounding, and converting it would be
     * undefined. */
    if (!(x == x))
        return 0;
    if (x >= 1073741824.0f)
...
/* On floats, so a source wider than eight bits - a GL_FLOAT or 16-bit image - is scaled
 * before it is quantised rather than after. Clamped at the end, as Mesa clamps. */
...
void gl_pixel_transfer_rgba8(const gl_context_t *ctx, uint8_t rgba[4]) {
    float c[4];
    for (int i = 0; i < 4; i++)
        c[i] = (float)rgba[i] / 255.0f;
    gl_pixel_transfer_rgbaf(ctx, c);
    for (int i = 0; i < 4; i++)
        rgba[i] = (uint8_t)(c[i] * 255.0f + 0.5f);
}

/* Recorded in lists; glPixelTransferi forwards here, so one operation covers both. */
void glPixelTransferf(GLenum pname, GLfloat param) {
    if (gl_list_recording() &&
        GL_LIST_REC(GL_LIST_OP_PIXEL_TRANSFER, gl_la_e(pname), gl_la_f(param)))
        return;
    gl_context_t *ctx = gl_get_ctx();
    if (!ctx)
        return;
    ...
    switch (pname) {
    case GL_MAP_COLOR:
        ctx->map_color = (GLboolean)(param != 0.0f);
        break;
    case GL_MAP_STENCIL:
        ctx->map_stencil = (GLboolean)(param != 0.0f);
        break;
    case GL_INDEX_SHIFT:
        ctx->index_shift = rounded;
        break;
    case GL_INDEX_OFFSET:
        ctx->index_offset = rounded;
        break;
    case GL_RED_SCALE:
        ctx->pixel_scale[0] = param;
        break;
    case GL_GREEN_SCALE:
        ctx->pixel_scale[1] = param;
        break;
    case GL_BLUE_SCALE:
        ctx->pixel_scale[2] = param;
        break;
    case GL_ALPHA_SCALE:
        ctx->pixel_scale[3] = param;
        break;
    case GL_RED_BIAS:
        ctx->pixel_bias[0] = param;
        break;
    case GL_GREEN_BIAS:
        ctx->pixel_bias[1] = param;
        break;
    case GL_BLUE_BIAS:
        ctx->pixel_bias[2] = param;
        break;
    case GL_ALPHA_BIAS:
        ctx->pixel_bias[3] = param;
        break;
    case GL_DEPTH_SCALE:
        ctx->depth_scale = param;
        break;
    case GL_DEPTH_BIAS:
        ctx->depth_bias = param;
        break;
    default:
        gl_record_error(ctx, GL_INVALID_ENUM);
        break;
...
/* Maps looked up by an index hold indices, and every other map holds colour components
 * - which decides both how the integer forms convert and how stored values are clamped.
 */
 ...
 * buffer bound.
 *
 * Stored as Mesa stores them: colour components clamped to 0..1, stencil indices
 * rounded, colour indices as given (store_pixelmap). */
void glPixelMapfv(GLenum map, GLsizei mapsize, const GLfloat *values) {
    if (gl_list_recording()) {
        ...
        if (keep) {
            ...
            if (!copy) {
                ...
                if (c && c->list_mode == GL_COMPILE)
                    return;
            } else {
                for (GLsizei i = 0; i < mapsize; i++)
                    ((GLfloat *)copy)[i] = values[i];
            }
        }
        if ((!keep || copy) &&
    ...
    for (GLsizei i = 0; i < mapsize; i++) {
        ...
        if (map == GL_PIXEL_MAP_S_TO_S)
            v = (float)gl_round_half_even(v);
        else if (map != GL_PIXEL_MAP_I_TO_I)
            v = gl_clamp01(v);
...
/* The integer forms convert and forward, so glPixelMapfv's checks and its list record
 * cover them too. An index map takes the integers as they are; a colour map takes them
 * normalised, the whole unsigned range onto 0..1 (Mesa main/pixel.c, UINT_TO_FLOAT and
 * USHORT_TO_FLOAT). */
...
/* Read back the way they were set: an index map as integers, a colour map normalised
 * over the type's range and rounded. Mesa's integer readbacks do otherwise for the
 * index maps - glGetPixelMapuiv copies S_TO_S's float bits and normalises I_TO_I
 * (main/pixel.c) - which does not give back what glPixelMapuiv put in; these do. */
...
void glGetPixelMapuiv(GLenum map, GLuint *values) {
    ...
    for (GLint i = 0; i < n; i++) {
        ...
        values[i] =
            v <= 0.0 ? 0u : (v >= 4294967295.0 ? 0xffffffffu : (GLuint)(v + 0.5));
    }
}
...
void glGetPixelMapusv(GLenum map, GLushort *values) {
    ...
    for (GLint i = 0; i < n; i++) {
        ...
        values[i] = v <= 0.0f
                        ? (GLushort)0
                        : (v >= 65535.0f ? (GLushort)0xffff : (GLushort)(v + 0.5f));
    }
}
