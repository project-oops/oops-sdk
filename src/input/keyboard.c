...
int oops_keyboard_init(void) {
    ...
    if (!s_keyboard_module_loaded) {
        if (sceSysmoduleLoadModule &&
            oops_symbol_is_resolved((const void *)sceSysmoduleLoadModule)) {
        ...
        if (sceKeyboardSetProcessFocus &&
            oops_symbol_is_resolved((const void *)sceKeyboardSetProcessFocus)) {
    ...
    if (!oops_keyboard_available()) {
        oops_log_warn(
            "KBD", "keyboard entry points unavailable (open=%p read=%p readState=%p)",
...
static uint8_t kbd_event_modifiers(const oops_kbd_hw_record_t *rec) {
    ...
    uint8_t m = 0u;
    if (p & 0x11u)
        m |= (uint8_t)OOPS_KMOD_CTRL; /* left | right */
    if (p & 0x22u)
        m |= (uint8_t)OOPS_KMOD_SHIFT;
    if (p & 0x44u)
        m |= (uint8_t)OOPS_KMOD_ALT;
    if (p & 0x88u)
        m |= (uint8_t)OOPS_KMOD_GUI;
    return m;
#else
    (void)rec;
    return 0u;
...
int oops_keyboard_read(oops_key_event_t *out_events, unsigned int max_events) {
    ...
    const unsigned int cap = (max_events > OOPS_MAX_KEY_EVENTS)
                                 ? (unsigned int)OOPS_MAX_KEY_EVENTS
                                 : max_events;
    unsigned int n = 0u;
