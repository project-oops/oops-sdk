...
int oops_dns_parse_response(const uint8_t *resp, size_t resp_len,
                            uint16_t expected_tx_id, char *out_ip, size_t out_len) {
...
int oops_net_resolve(const char *hostname, char *out_ip, size_t out_len) {
    ...
    if (getaddrinfo(hostname, NULL, &hints, &res) == 0 && res) {
        ...
        const char *ptr =
            inet_ntop(AF_INET, &(ipv4->sin_addr), out_ip, (socklen_t)out_len);
        freeaddrinfo(res);
        if (ptr) {
    ...
    if (oops_net_ctl_get_info(&net_info) == 0) {
        if (net_info.primary_dns[0] != '\0')
            servers[num_servers++] = net_info.primary_dns;
        if (net_info.secondary_dns[0] != '\0')
            servers[num_servers++] = net_info.secondary_dns;
    }
    ...
    for (int s = 0; s < num_servers; s++) {
        ...
        while (oops_time_get_ms() - start_ms < 1500) {
            ...
            if (r > 0) {
                if (oops_dns_parse_response(resp, (size_t)r, tx_id, out_ip, out_len) ==
                    0) {
                    success = 1;
                }
                break;
        ...
        oops_close(sock);
        if (success) {
            oops_log_info("DNS", "resolved '%s' -> %s (via %s)", hostname, out_ip,
                          servers[s]);
            return 0;
        }
    }

    oops_log_warn("DNS", "failed to resolve host '%s' across %d servers", hostname,
                  num_servers);
    return -1;
#endif
}
