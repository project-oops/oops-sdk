...
int oops_http_url_parse(const char *url,
                        char *out_scheme, size_t scheme_sz,
  ...
  if (*p == ':') {
    ...
    while (*p >= '0' && *p <= '9') {
      ...
      digits++;
      p++;
      if (val > 65535 || digits > 5) return -1;
    }
...
int oops_http_build_request(const char *host, const char *path,
...
int oops_http_parse_headers(const char *hdr_buf, size_t hdr_len,
                            int *out_status, size_t *out_content_len,
                            int *out_is_chunked,
                            char *out_content_type, size_t ct_sz,
                            char *out_location, size_t loc_sz,
                            size_t *out_hdr_end_offset) {
  ...
  if (out_content_len) *out_content_len = (size_t)-1;
  if (out_is_chunked) *out_is_chunked = 0;
  if (out_content_type && ct_sz > 0) out_content_type[0] = '\0';
  if (out_location && loc_sz > 0) out_location[0] = '\0';
  ...
  while (line < end) {
    ...
    if (line_len > 15 && strncasecmp_ascii(line, "Content-Length:", 15) == 0) {
      ...
      if (out_content_len) *out_content_len = clen;
    } else if (line_len > 18 && strncasecmp_ascii(line, "Transfer-Encoding:", 18) == 0) {
      const char *val = line + 18;
      while (val < line_end && (*val == ' ' || *val == '\t')) val++;
      if (strncasecmp_ascii(val, "chunked", 7) == 0) {
        if (out_is_chunked) *out_is_chunked = 1;
      }
      ...
      if (out_content_type && ct_sz > 0) {
        ...
        for (size_t k = 0; k < copy_len; k++) out_content_type[k] = val[k];
      ...
      if (out_location && loc_sz > 0) {
        ...
        for (size_t k = 0; k < copy_len; k++) out_location[k] = val[k];
...
int oops_http_decode_chunked(const char *src, size_t src_len,
  ...
  while (p < end) {
    ...
    while (p < end) {
      ...
      else if (c >= 'a' && c <= 'f') digit_val = 10 + (c - 'a');
      else if (c >= 'A' && c <= 'F') digit_val = 10 + (c - 'A');
      else break;
...
__attribute__((weak)) int sceHttpCreateTemplate(int httpCtxId, const char *userAgent, int httpVer, int isAutoRedirect);
__attribute__((weak)) int sceHttpDeleteTemplate(int tmplId);
__attribute__((weak)) int sceHttpCreateConnectionWithURL(int tmplId, const char *url, int isEnableKeepalive);
__attribute__((weak)) int sceHttpDeleteConnection(int connId);
__attribute__((weak)) int sceHttpCreateRequestWithURL2(int connId, const char *method, const char *path_or_url, uint64_t contentLength);
__attribute__((weak)) int sceHttpDeleteRequest(int reqId);
__attribute__((weak)) int sceHttpAddRequestHeader(int id, const char *name, const char *value, int mode);
__attribute__((weak)) int sceHttpSetAutoRedirect(int reqId, int enable);
__attribute__((weak)) int sceHttpSendRequest(int reqId, const void *postData, size_t size);
__attribute__((weak)) int sceHttpGetStatusCode(int reqId, int *statusCode);
__attribute__((weak)) int sceHttpGetResponseContentLength(int reqId, int *result, uint64_t *contentLength);
...
static int oops_http_tls_get(const char *url, oops_http_response_t *out_resp) {
  ...
  int http_ctx = sceHttpInit(net_pool > 0 ? net_pool : 0,
                             ssl_ctx > 0 ? ssl_ctx : 0,
                             256 * 1024);
  if (http_ctx <= 0) {
    oops_log_warn("HTTP", "sceHttpInit failed (http_ctx=%d)", http_ctx);
    if (ssl_ctx > 0 && sceSslTerm) sceSslTerm(ssl_ctx);
    if (net_pool > 0 && sceNetPoolDestroy) sceNetPoolDestroy(net_pool);
  ...
  if (sceHttpGetResponseContentLength) {
    if (sceHttpGetResponseContentLength(req, &res_info, &clen) == 0 && res_info == 0) {
      out_resp->content_length = (size_t)clen;
    }
  }
  oops_log_debug("HTTP", "TLS response: status=%d clen=%llu", status_code, (unsigned long long)clen);
...
static int oops_http_tls_get_to_file(const char *url, const char *dest_path,
  ...
  while (1) {
    ...
    if (written < 0 || (size_t)written != (size_t)n) {
      oops_log_error("HTTP", "write error saving to %s: %ld != %d", dest_path, written, n);
      oops_fs_close(fd);
      sceHttpDeleteRequest(req);
      sceHttpDeleteConnection(conn);
      sceHttpDeleteTemplate(tmpl);
    ...
    if (on_progress) on_progress((uint64_t)total_written, 0, userdata);
  }
  ...
  oops_log_info("HTTP", "TLS GET %s saved to %s (%zu bytes)", url, dest_path, total_written);
  return OOPS_HTTP_OK;
}
...
int oops_http_get(const char *url, oops_http_response_t *out_resp) {
  ...
  if (oops_http_url_parse(url, scheme, sizeof(scheme), host, sizeof(host),
  ...
  while (1) {
    ...
    if (!headers_parsed) {
      if (oops_http_parse_headers(raw_buf, raw_len, &status_code, &content_len,
                                  &is_chunked, out_resp->content_type,
                                  sizeof(out_resp->content_type),
                                  out_resp->location,
                                  sizeof(out_resp->location), &hdr_end) == 0) {
        headers_parsed = 1;
        out_resp->status_code = status_code;
    ...
    if (raw_len + 4096 >= buf_cap) {
      ...
      for (size_t i = 0; i < raw_len; i++) new_buf[i] = raw_buf[i];
      http_free(raw_buf);
      raw_buf = new_buf;
    }
  }
  ...
  if (is_chunked) {
    ...
    size_t dec_len = 0;
    if (oops_http_decode_chunked(body_src, body_raw_len, decoded, body_raw_len + 1, &dec_len) != 0) {
    ...
    if (!body) {
      oops_log_error("HTTP", "failed to allocate buffer for body (%zu bytes)", body_raw_len + 1);
      http_free(raw_buf);
      return OOPS_HTTP_ERR_NOMEM;
    }
    for (size_t i = 0; i < body_raw_len; i++) body[i] = body_src[i];
    body[body_raw_len] = '\0';
    http_free(raw_buf);
    out_resp->body = body;
  ...
  oops_log_info("HTTP", "GET %s -> %d (%zu bytes)", url, out_resp->status_code, out_resp->body_size);
  return OOPS_HTTP_OK;
}
...
int oops_http_get_to_file_cb(const char *url, const char *dest_path,
  ...
  if (status_code != 200) {
    oops_log_warn("HTTP", "GET %s returned status %d (expected 200)", url, status_code);
    oops_close(sock);
    return OOPS_HTTP_ERR_PARSE;
  }
  ...
  if (hdr_len > hdr_end) {
    ...
    if (written > 0) total_written += (size_t)written;
  }

  uint64_t total_expected = (content_len == (size_t)-1) ? 0 : (uint64_t)content_len;
  if (on_progress) on_progress((uint64_t)total_written, total_expected, userdata);
  ...
  oops_fs_close(fd);
  oops_close(sock);
  oops_log_info("HTTP", "GET %s saved to %s (%zu bytes)", url, dest_path, total_written);
  return OOPS_HTTP_OK;
}
