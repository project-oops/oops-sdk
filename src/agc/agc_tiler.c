...
int agc_tiler_dispatch_params(uint32_t *user_data, uint32_t *groups_x,
    ...
    if (agc_buffer_descriptor(src_desc, linear_src, 4u, width * height,
                              AGC_BUF_FMT_32_UINT) != 0) {
        return -1;
    }
    if (agc_buffer_descriptor(dst_desc, tiled_dst, 8u, (uint32_t)(tiled_bytes / 8u),
