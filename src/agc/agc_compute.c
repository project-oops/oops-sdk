...
static oops_gpu_queue_t *create_queue_type(uint32_t type) {
    ...
    oops_gpu_queue_t *q = (oops_gpu_queue_t *)oops_mem_alloc(sizeof(oops_gpu_queue_t),
                                                             16, OOPS_MEM_WB_ONION);
    if (!q)
        return (oops_gpu_queue_t *)0;
    ...
    q->dcb_size = 0x10000u;
    q->dcb_mem = (uint8_t *)oops_mem_alloc(q->dcb_size, 0x1000, OOPS_MEM_WB_ONION);

    /* Allocate coherent Onion WB memory for completion fence */
    q->fence = (volatile uint32_t *)oops_mem_alloc(0x1000, 0x1000, OOPS_MEM_WB_ONION);
    ...
    *q->fence = 0;
    q->last_fence = 0;
    oops_log_debug("AGC", "create_queue_type(%u) succeeded: handle=%p", type,
                   queue_handle);
    return q;
}
...
int oops_gpu_dispatch(oops_gpu_queue_t *queue, const oops_gpu_dispatch_t *dispatch) {
    ...
    *dw++ = 0u;
    ...
    oops_log_trace("AGC", "dispatch grid=(%u,%u,%u) dwords=%u fence=%u", gx, gy, gz,
                   dcb_dwords, next_fence);
...
oops_gpu_shader_t *oops_gpu_create_shader(const void *hdr, size_t hs, const void *p,
                                          size_t ps) {
    (void)hdr;
    (void)hs;
    (void)p;
...
void oops_gpu_destroy_shader(oops_gpu_shader_t *sh) {
    (void)sh;
}
