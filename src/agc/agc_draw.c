...
int oops_agc_draw_primitive(oops_gpu_queue_t *queue, const oops_agc_draw_desc_t *desc) {
    if (!queue || !desc || !desc->color_buffer || desc->width == 0 ||
        desc->height == 0) {
        return -1;
    }
    ...
    uint32_t prim_type =
        desc->primitive_type ? desc->primitive_type : OOPS_AGC_PRIM_TRILIST;
    uint32_t vcount = desc->vertex_count ? desc->vertex_count : 3;

    oops_log_trace(
        "AGC", "draw primitive: prim=%u verts=%u size=%ux%u ngg_va=0x%llx ps_va=0x%llx",
        prim_type, vcount, desc->width, desc->height, (unsigned long long)ngg_va,
        (unsigned long long)ps_va);
    ...
    *queue->fence = 0x11111111u;
    ...
    for (size_t i = 0; i < sizeof(base_ctx_regs) / sizeof(base_ctx_regs[0]); i++) {
        ...
        if (reg == 0x318u) {
            ...
            val =
                (desc->depth_buffer && desc->depth_control) ? desc->depth_control : 0u;
        } else if (reg == 0x203u) {
            val = 0x00000000u; /* DB_SHADER_CONTROL: LATE_Z default matching
                                  AgcCompositor.elf */
    ...
    if (desc->depth_buffer && desc->depth_control) {
        ...
        uint32_t z_format =
            desc->depth_format ? (desc->depth_format & 0x3u) : OOPS_AGC_Z_32_FLOAT;
        uint32_t z_info =
            z_format |
            0x80000180u; /* SW_MODE=24 (0x180), ZRANGE_PRECISION (0x80000000) */
        ...
        for (size_t i = 0; i < sizeof(db_regs) / sizeof(db_regs[0]); i++) {
            ...
            if (reg == OOPS_AGC_REG_DB_DEPTH_SIZE_XY) {
                ...
                val = z_info;
                       ...
                       reg == OOPS_AGC_REG_DB_Z_WRITE_BASE) {
                val = (uint32_t)(depth_gpu >> 8);
            } else if (reg == OOPS_AGC_REG_DB_Z_READ_BASE_HI ||
                       reg == OOPS_AGC_REG_DB_Z_WRITE_BASE_HI) {
    ...
    *dw++ = 0u;
    ...
    oops_log_warn("AGC",
                  "draw primitive fence timeout waiting for 0xbeefcafe (fence=0x%x)",
                  *queue->fence);
