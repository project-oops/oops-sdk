...
static void *sys_vm_alloc(size_t size) {
  ...
  long ret = sys_call(SYS_mmap, 0, (long)size, OOPS_PROT_RW, OOPS_MAP_ANON_PRIV, -1, 0);
...
void *oops_malloc(size_t size) {
  ...
  if (class_idx >= 0) {
    ...
    heap_block_header_t *hdr = (heap_block_header_t *)((uint8_t *)node - sizeof(heap_block_header_t));
