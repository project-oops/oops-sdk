...
int oops_mem_alloc_direct(size_t size, size_t alignment, oops_mem_type_t type,
    ...
    if (!out_phys || size == 0)
        return -1;
    oops_log_trace("MEM", "alloc direct: size=%zu align=0x%zx type=%d", size, alignment,
                   (int)type);
    sce_off_t p = 0;
    int rc = -1;
    if (sceKernelAllocateMainDirectMemory) {
        ...
        rc = sceKernelAllocateDirectMemory(0, (sce_off_t)sceKernelGetDirectMemorySize(),
                                           size, alignment, (int)type, &p);
    }
    if (rc == 0) {
        *out_phys = (int64_t)p;
        oops_log_debug("MEM", "alloc direct ok: phys=0x%llx size=%zu",
                       (unsigned long long)p, size);
    } else {
        oops_log_warn("MEM", "alloc direct failed: rc=%d size=%zu type=%d", rc, size,
                      (int)type);
    }
    return rc;
}
...
int oops_mem_free_direct(int64_t phys, size_t size) {
    oops_log_trace("MEM", "free direct: phys=0x%llx size=%zu", (unsigned long long)phys,
                   size);
    ...
    if (rc != 0) {
        oops_log_warn("MEM", "free direct failed: rc=%d phys=0x%llx", rc,
...
int oops_mem_map_direct(void **out_vaddr, size_t size, int prot, int flags,
    ...
    if (!out_vaddr || !sceKernelMapDirectMemory)
        return -1;
    oops_log_trace("MEM",
                   "map direct: phys=0x%llx size=%zu prot=0x%x flags=0x%x align=0x%zx",
                   (unsigned long long)phys, size, prot, flags, alignment);
    void *v = *out_vaddr;
    int rc =
        sceKernelMapDirectMemory(&v, size, prot, flags, (sce_off_t)phys, alignment);
    if (rc == 0) {
        *out_vaddr = v;
        oops_log_debug("MEM", "map direct ok: vaddr=%p phys=0x%llx size=%zu", v,
                       (unsigned long long)phys, size);
    } else {
        oops_log_warn("MEM", "map direct failed: rc=%d phys=0x%llx size=%zu", rc,
                      (unsigned long long)phys, size);
    }
    return rc;
}
...
int oops_mem_reserve_va(void **addr_inout, size_t len, int flags, size_t alignment) {
    if (!addr_inout || len == 0 || !sceKernelReserveVirtualRange)
        return -1;
    oops_log_trace("MEM", "reserve VA: len=%zu flags=0x%x align=0x%zx", len, flags,
                   alignment);
...
int oops_mem_release_va(void *vaddr, size_t len) {
    ...
    if (rc != 0) {
        oops_log_warn("MEM", "release VA failed: rc=%d vaddr=%p len=%zu", rc, vaddr,
                      len);
    }
    return rc;
}
...
int oops_mem_batch_map(void *vaddr_base, int64_t phys_base, size_t total_size,
    ...
    if (!sceKernelBatchMap || !vaddr_base || page_size == 0 || total_size == 0)
        return -1;
    oops_log_trace(
        "MEM", "batch map: vaddr=%p phys=0x%llx total_size=%zu page_size=%zu prot=0x%x",
        vaddr_base, (unsigned long long)phys_base, total_size, page_size,
        (unsigned)prot);
    ...
    while (page_idx < total_pages) {
        ...
        for (size_t i = 0; i < batch; i++) {
            ...
            entries[i].paddr = (sce_off_t)(phys_base + (int64_t)p * (int64_t)page_size);
        ...
        if (completed != (int)batch) {
            oops_log_warn("MEM", "batch map incomplete: completed %d of %zu", completed,
                          batch);
            return -1;
        }
        page_idx += batch;
...
void *oops_mem_alloc(size_t size, size_t alignment, oops_mem_type_t type) {
    ...
    oops_log_trace("MEM",
                   "oops_mem_alloc: req_size=%zu aligned_size=%zu align=0x%zx type=%d",
    ...
    if (slot_idx < 0) {
        oops_log_warn("MEM", "oops_mem_alloc: out of tracking slots (max %d)",
                      OOPS_MAX_ALLOCS);
        return NULL;
    }
