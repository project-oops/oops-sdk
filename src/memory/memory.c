...
int oops_mem_map_direct(void **out_vaddr, size_t size, int prot, int flags, int64_t phys, size_t alignment) {
    if (!out_vaddr || !sceKernelMapDirectMemory) return -1;
    void *v = NULL;
    int rc = sceKernelMapDirectMemory(&v, size, prot, flags, (sce_off_t)phys, alignment);
    if (rc == 0) {
        *out_vaddr = v;
    }
    return rc;
...
int oops_mem_unmap(void *vaddr, size_t size) {
    if (!sceKernelMunmap || !vaddr) return -1;
