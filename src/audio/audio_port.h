/*
 * The audio port, private to the audio subsystem and its host tests. The chunking in
 * oops_audio_write() is the whole of what this subsystem computes, so the tests build a port
 * here with a recording sink in place of the platform's output call and drive it directly.
 * Nothing in this file is API.
 */
...
struct oops_audio_port {
    ...
    int16_t staging[OOPS_AUDIO_MAX_CHUNK * OOPS_AUDIO_CHANNELS] __attribute__((aligned(64)));
};
