...
__attribute__((weak)) int sceAudioOutSetVolume(int handle, int flag,
                                               int *volumes);
...
int oops_audio_get_last_error(void) { return s_last_audio_error; }
...
oops_audio_port_t *oops_audio_open(int sample_rate, int channels,
  ...
  if (user >= 0) {
    handle = sceAudioOutOpen(user, 0, 0, (unsigned int)frames,
                             (unsigned int)rate, OOPS_AUDIO_FORMAT_PARAM);
  }
  /* Fallback with system user 0xFF if failed */
  if (handle < 0) {
    handle = sceAudioOutOpen(0xFF, 0, 0, (unsigned int)frames,
                             (unsigned int)rate, OOPS_AUDIO_FORMAT_PARAM);
  }
  if (handle < 0) {
...
int oops_audio_write(oops_audio_port_t *port, const int16_t *pcm_samples,
                     size_t frame_count) {
  if (!port || port->handle < 0 || !port->sink || !pcm_samples ||
      frame_count == 0) {
    return -1;
  }
