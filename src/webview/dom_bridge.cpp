...
static void js_element_finalizer(JSRuntime *rt, JSValue val) {
    auto *data = static_cast<dom_element_data *>(JS_GetOpaque(val, js_element_class_id));
    if (data) {
...
static JSValue js_element_append_child(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    auto *data = static_cast<dom_element_data *>(JS_GetOpaque(this_val, js_element_class_id));
    auto *child_data = static_cast<dom_element_data *>(JS_GetOpaque(argv[0], js_element_class_id));
...
static JSValue js_element_set_attribute(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    auto *data = static_cast<dom_element_data *>(JS_GetOpaque(this_val, js_element_class_id));
    if (!data || !data->el) return JS_UNDEFINED;
...
static JSValue js_element_get_attribute(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    auto *data = static_cast<dom_element_data *>(JS_GetOpaque(this_val, js_element_class_id));
    if (!data || !data->el) return JS_NULL;
...
static JSValue js_element_has_attribute(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    auto *data = static_cast<dom_element_data *>(JS_GetOpaque(this_val, js_element_class_id));
    if (!data || !data->el) return JS_NewBool(ctx, 0);
...
static JSValue js_element_add_event_listener(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    if (argc < 2) return JS_UNDEFINED;
    auto *data = static_cast<dom_element_data *>(JS_GetOpaque(this_val, js_element_class_id));
    if (!data || !JS_IsFunction(ctx, argv[1])) return JS_UNDEFINED;
...
static JSValue js_element_query_selector_all(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    auto *data = static_cast<dom_element_data *>(JS_GetOpaque(this_val, js_element_class_id));
    if (!data || !data->el) return JS_NewArray(ctx);
...
static JSValue js_element_get_text_content(JSContext *ctx, JSValueConst this_val) {
    auto *data = static_cast<dom_element_data *>(JS_GetOpaque(this_val, js_element_class_id));
    if (!data || !data->el) return JS_NewString(ctx, "");
...
static JSValue js_element_set_text_content(JSContext *ctx, JSValueConst this_val, JSValueConst val) {
    auto *data = static_cast<dom_element_data *>(JS_GetOpaque(this_val, js_element_class_id));
    if (!data || !data->el) return JS_UNDEFINED;
...
static JSValue js_element_set_inner_html(JSContext *ctx, JSValueConst this_val, JSValueConst val) {
    auto *data = static_cast<dom_element_data *>(JS_GetOpaque(this_val, js_element_class_id));
    if (!data || !data->el) return JS_UNDEFINED;
...
static JSValue js_element_set_class_name(JSContext *ctx, JSValueConst this_val, JSValueConst val) {
    auto *data = static_cast<dom_element_data *>(JS_GetOpaque(this_val, js_element_class_id));
    if (!data || !data->el) return JS_UNDEFINED;
...
static const JSCFunctionListEntry js_element_proto_funcs[] = {
    ...
    JS_CGETSET_DEF("textContent", js_element_get_text_content, js_element_set_text_content),
...
static JSValue js_document_query_selector_all(JSContext *ctx, JSValueConst /*this_val*/, int argc, JSValueConst *argv) {
    if (argc < 1) return JS_NewArray(ctx);
    auto *wv = static_cast<oops_webview_t *>(JS_GetContextOpaque(ctx));
    if (!wv || !wv->html) return JS_NewArray(ctx);

    auto *doc = static_cast<litehtml::document *>(oops_html_get_document(wv->html));
    if (!doc || !doc->root()) return JS_NewArray(ctx);
...
static JSValue js_window_add_event_listener(JSContext *ctx, JSValueConst /*this_val*/, int argc, JSValueConst *argv) {
    if (argc < 2) return JS_UNDEFINED;
    auto *wv = static_cast<oops_webview_t *>(JS_GetContextOpaque(ctx));
    if (!wv || !JS_IsFunction(ctx, argv[1])) return JS_UNDEFINED;

    const char *event_type = JS_ToCString(ctx, argv[0]);
    if (event_type) {
        wv->window_listeners.push_back({ JS_DupValue(ctx, argv[1]), std::string(event_type) });
        JS_FreeCString(ctx, event_type);
    }
    return JS_UNDEFINED;
}
...
int dom_bridge_init(JSContext *ctx, oops_webview_t *wv, oops_container * /*container*/) {
    ...
    JS_SetPropertyStr(ctx, doc_obj, "getElementById", JS_NewCFunction(ctx, js_document_get_element_by_id, "getElementById", 1));
    JS_SetPropertyStr(ctx, doc_obj, "querySelector", JS_NewCFunction(ctx, js_document_query_selector, "querySelector", 1));
    JS_SetPropertyStr(ctx, doc_obj, "querySelectorAll", JS_NewCFunction(ctx, js_document_query_selector_all, "querySelectorAll", 1));
    JS_SetPropertyStr(ctx, doc_obj, "createElement", JS_NewCFunction(ctx, js_document_create_element, "createElement", 1));
    JS_SetPropertyStr(ctx, doc_obj, "addEventListener", JS_NewCFunction(ctx, js_window_add_event_listener, "addEventListener", 2));
    ...
    JS_SetPropertyStr(ctx, global_obj, "addEventListener", JS_NewCFunction(ctx, js_window_add_event_listener, "addEventListener", 2));

    JS_FreeValue(ctx, doc_obj);
    JS_FreeValue(ctx, global_obj);

    oops_log_debug("WEBVIEW", "DOM bridge initialized with Document, Window, and Element prototypes");
    return 0;
}

void dom_bridge_dispatch_event(JSContext *ctx, JSValue elem_obj, const char *event_type, JSValue event_obj) {
    if (!event_type) return;
    auto *data = static_cast<dom_element_data *>(JS_GetOpaque(elem_obj, js_element_class_id));
    if (!data) return;
    ...
    for (const auto& l : data->listeners) {
        if (l.event_type == event_type) {
            ...
            if (JS_IsException(ret)) {
                ...
                oops_log_error("WEBVIEW", "Event listener error: %s", err ? err : "unknown");
