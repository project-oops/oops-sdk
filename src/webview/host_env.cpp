...
static JSValue js_console_log_impl(JSContext *ctx, JSValueConst /*this_val*/, int argc,
    ...
    switch (level) {
    case 0:
        oops_log_debug("WEBVIEW", "%s", msg.c_str());
        break;
    case 1:
        oops_log_info("WEBVIEW", "%s", msg.c_str());
        break;
    case 2:
        oops_log_warn("WEBVIEW", "%s", msg.c_str());
        break;
    case 3:
        oops_log_error("WEBVIEW", "%s", msg.c_str());
        break;
    default:
        oops_log_info("WEBVIEW", "%s", msg.c_str());
        break;
...
static JSValue js_clear_timeout(JSContext *ctx, JSValueConst /*this_val*/, int argc,
                                JSValueConst *argv) {
    if (argc < 1)
        return JS_UNDEFINED;
    auto *wv = static_cast<oops_webview_t *>(JS_GetContextOpaque(ctx));
...
static JSValue js_clear_interval(JSContext *ctx, JSValueConst this_val, int argc,
                                 JSValueConst *argv) {
...
static JSValue js_fetch(JSContext *ctx, JSValueConst /*this_val*/, int argc,
    ...
    auto *wv = static_cast<oops_webview_t *>(JS_GetContextOpaque(ctx));
    if (!wv)
        return JS_ThrowInternalError(ctx, "No webview context");
        ...
        if (now_ms >= active[i].next_fire_ms) {
            ...
            if (it != wv->timers.end()) {
                ...
                if (JS_IsException(ret)) {
                    ...
                    oops_log_error("WEBVIEW", "Timer callback exception: %s",
                                   err ? err : "unknown");
                    if (err)
...
void host_env_pump_fetches(JSContext *ctx, oops_webview_t *wv) {
    if (!wv || wv->pending_fetches.empty())
        return;
    ...
    for (auto &task : current_fetches) {
        ...
        if (err != OOPS_HTTP_OK) {
            ...
            JS_SetPropertyStr(ctx, res_obj, "status",
                              JS_NewInt32(ctx, resp.status_code));
            JS_SetPropertyStr(
                ctx, res_obj, "ok",
                JS_NewBool(ctx, resp.status_code >= 200 && resp.status_code < 300));
            JS_SetPropertyStr(ctx, res_obj, "statusText",
                              JS_NewString(ctx, resp.status_code == 200 ? "OK" : ""));
            JS_SetPropertyStr(ctx, res_obj, "_body",
                              JS_NewString(ctx, resp.body ? resp.body : ""));
            JS_SetPropertyStr(ctx, res_obj, "text",
                              JS_NewCFunction(ctx, js_response_text, "text", 0));
            JS_SetPropertyStr(ctx, res_obj, "json",
