...
| GLSL 1.20's whole-array assignment and comparison | **done** since 2026-09-25, in both back ends - which completes 1.20's arrays and the dialect with them. `v = w` and `v == w`. Neither needed a new storage shape: an array is already its elements end to end on both paths, so the work was letting a whole array *be* a place and *be* a value, which each back end refused on the grounds that the language had no array-valued expression. It has one now, in these two rules and the constructors, and nowhere else. **The length is the part neither back end can see**: an expression here carries a type and only a symbol carries a length, and a whole array's type is its *element*'s - so `v == w` on two `float[2]` reaches the operator as FLOAT against FLOAT, and the semantic stage has to ask about the arrays before comparing element types or two arrays of different lengths agree on their first element. The interpreter's values carry their own width for the same reason; without it `q = p` on a `vec2[3]` copied one element and zeroed the rest, which reads as an assignment that happened and a comparison that then disagrees. 1.10 keeps both refusals (5.8 and 5.9) by version, and a compound assignment on an array stays refused in both - 1.20 gives arrays `=`, not arithmetic |
...
| Loops in a compiled shader | **done** - every shape GLSL can write. A counted `for` is unrolled where it fits, which keeps the counter a compile-time constant so indexing and arithmetic on it fold away; anything else takes the branched path, three scalar masks and a trip guard. `while`, `do`-`while`, a `for` over a counter declared outside, one whose bound is a uniform, one whose body moves its own counter, and `for (;;)` with a `break` are all that second case. **One shape stays refused**, and the distinction is the point: a loop whose trip count is *known* and larger than `GLSL_GEN_MAX_TRIPS` would run the ceiling's worth of trips and draw a wrong colour with no error. That is a truncation rather than a bound. Every other shape falls through because its count is unknown, where the guard is the best answer available and a shader that does what it says never reaches it. **A counted loop left early by a `break` was measured wrong on hardware and is not any more**: censused at 1484 of 3072 pixels wrong on 2026-09-24, while the same shader with no loop, one trip, and a plain counted loop all censused 0 (obSCEne `REQ-...-5b8e` update 12, isolated from the blend fault and accepted as ours). `gl2-probe`'s `loop-uniformity/break` is that shape - `for (k = 0; k < 8; k++) { if (k == 1) break; b += 0.25; }` - and it censuses clean on hardware as of the 2026-09-25 run, 067/067. Nothing since 09-24 names it as the thing it fixed, so which commit closed it is not known; what is known is the before and the after, measured the same way. **No host gate could have told us either way** - `loop-uniformity` passes on the software reference and in the simulator, so this was console-only in both directions.

**What is still not exercised: a break that fires for some lanes and not others.** `loop-uniformity/break-divergent` is written so that no lane breaks before the last trip - its divergence is in the condition being per-pixel, not in the count - so a genuinely divergent early exit, where `exec` has to come back with some lanes gone, has no arm |
