...
| Loops in a compiled shader | not started - `for` and `while` are the one thing here that a mask cannot express, because the trip count differs per lane. They need a real branch, a label mechanism and a loop-exit mask, none of which the generator has. A shader with a loop runs on the reference and is refused for the console |
