...
**Sixty-five of those passed on the console** on 2026-09-25, which is the property the suite
exists for - a check that passes on the reference and fails on the hardware is a hardware-path
defect. The sixty-sixth is `framebuffer-objects`, added after that run and not yet measured
there; its two arms are separate so that one console run says whether the command processor
takes a renderbuffer, a texture, or neither.
