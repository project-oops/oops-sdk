...
against which `gl2-probe` measures 65 checks and `gl2-cube` draws a cube. Every one of those 65
passes on the console as well as on the reference, which is the property the suite exists for -
a check that passes in one place and fails in the other is a hardware-path defect.
...
| Instruction selection from the AST | **started** - `src/gl/glsl_gen.c`. Arithmetic including `/`, swizzle reads and writes, plain and compound assignment, constructors, declarations, blocks, user-defined functions, `struct` in every position a value takes, and the built-in library - the common, exponential and geometric functions of GLSL section 8. Everything else sets `error` and emits nothing, because an instruction whose encoding has not been read out of an assembler is not guessed |
...
| Texture sampling in a compiled shader | **`texture2D` is generated** - the shader loads its own image and sampler descriptors out of the same block the uniforms come from, samples with `image_sample` (not `_lz`, so the mip chain and the minification filter are used), and waits on `vmcnt` before reading the result. Two descriptor sets a draw, assigned to samplers at link time so the compiler and the draw path cannot disagree about which is which. `REQ-20260921T1830Z-2a45` asks hardware whether the texel comes back. **The projective, cube, volume and shadow lookups are generated too** - `texture2DProj`, `textureCube`, `texture3D`, `texture3DProj`, `shadow2D` and `shadow2DProj` |
...
| Loops in a compiled shader | **a bounded `for` is unrolled**; everything else is refused for the console and runs on the reference. Unrolling is available exactly when the trip count is known at compile time - the initialiser declares the counter, the increment is constant, and the body does not assign the counter - and each refusal names which of those failed rather than reporting the loop as unsupported. What is left is the case unrolling cannot reach: a trip count that differs per lane, which needs a real branch, a label mechanism and a loop-exit mask. `while` and `do-while` are that case by construction |
| Framebuffer objects | **the object layer and the software path are done**; the console is not. Names, storage, attachments, the completeness rules and `glGenerateMipmap` are in, and a draw into a bound framebuffer object lands in its attachment - the colour target, the depth buffer and the addressing width all follow it, which is what lets an attachment be a different size from the display. On the console `glCheckFramebufferStatus` answers GL_FRAMEBUFFER_UNSUPPORTED, because an attachment is linear RGBA8 in process memory and the hardware path builds its target descriptor from the display's surface. The status query calls the same predicate the draw path calls, so what it promises and what a draw does cannot drift |
