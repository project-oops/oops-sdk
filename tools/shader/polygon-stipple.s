.text
// The polygon stipple's discard (GL 1.0, 3.5.2). RDNA2 has no stipple hardware: radeonsi
// discards in the pixel shader, looking the fragment's window position up in the 32x32 mask,
// and this is that lookup.
//
// The fragment's position arrives because the draw sets SPI_PS_INPUT_ENA and
// SPI_PS_INPUT_ADDR to 0x302 - PERSP_CENTER_ENA with POS_X_FLOAT_ENA and POS_Y_FLOAT_ENA: v0 and
// v1 stay the barycentrics, v2 arrives as POS_X and v3 as POS_Y, at pixel centres. Nothing else
// is enabled, so v4 and v5 are undefined there - this reads neither.
//
// It sits at word 16 of both pixel shaders, after the canary block and before the
