...
static size_t tiled_offset(uint32_t w, uint32_t x, uint32_t y) {
    const size_t block =
        (size_t)(y >> 7) * (size_t)((w + 127u) / 128u) + (size_t)(x >> 7);
    return block * 65536u + (size_t)agc_tile_pixel(x & 127u, y & 127u) * 4u;
}

static int surface_init(surface_t *s, const char *name, uint32_t attrib3, int tiled,
                        size_t size) {
    s->name = name;
    s->attrib3 = attrib3;
    s->tiled = tiled;
...
static uint32_t px(const surface_t *s, uint32_t w, uint32_t x, uint32_t y,
                   int as_tiled) {
...
/* The count the check itself reports, for the shape mode to agree with: an independent
 * figure, since it comes from the probe's own sweep of the target rather than from
 * these rows. */
static uint32_t s_modified[MAX_ARMS + 1];
static const char *s_modified_of[MAX_ARMS + 1];
static int s_modified_n;

/* `OBS|bytes|<check>|<arm>|<key>|<offset>|<hex>`, the way 166-agc/primitive-draw writes
 * them. */
static void take_row(job_t *j, char *line) {
    ...
    if (strcmp(f[1], "measure") == 0 && strcmp(f[2], j->check) == 0 &&
        ...
        for (int a = 0; a < s_modified_n; a++) {
            if (s_modified_of[a] && strcmp(s_modified_of[a], f[3]) == 0)
                return;
        }
    ...
    if (strcmp(f[1], "bytes") != 0)
        return;
    if (strcmp(f[2], j->check) != 0 || strcmp(f[4], j->key) != 0)
        return;
    surface_t *s = NULL;
    if (strcmp(f[3], j->control.name) == 0)
        s = &j->control;
    for (int a = 0; a < j->n_arms && !s; a++) {
        if (strcmp(f[3], j->arms[a].name) == 0)
            s = &j->arms[a];
    }
    if (!s)
        return;
...
static uint32_t at_block(const surface_t *s, uint32_t blk, uint32_t x, uint32_t y) {
    return at32(s,
                (size_t)blk * 65536u + (size_t)agc_tile_pixel(x & 127u, y & 127u) * 4u);
}
 ...
 * rows of blocks moves entire image rows without splitting any of them, so every row
 * still holds one run and "one run a row" says nothing; but a triangle's rows are
 * contiguous, and moving half of them to the other end of the image leaves a hole in
 * the middle.
 */
static void shape_of(const job_t *j, const surface_t *s, const uint32_t *perm,
                     uint32_t *clean, uint32_t *drawn, uint32_t *touched,
                     uint32_t *gaps) {
...
/* Every permutation of the blocks, for a surface small enough to enumerate them.
 * Returns the number that give a clean shape, and writes the first one into `found`. */
static int shape_search(const job_t *j, const surface_t *s, FILE *out,
                        uint32_t *found) {
    const uint32_t nb = ((j->w + 127u) / 128u) * ((j->h + 127u) / 128u);
    if (nb == 0u || nb > 4u) {
        fprintf(
            out,
            "  block order: %u blocks is too many to enumerate; only the tool's own "
            "order is checked\n",
            nb);
    }
    ...
    for (;;) {
        ...
        if (touched && clean == touched && gaps == 0u) {
            ...
            fprintf(out,
                    "  block order %u %u %u %u: %u of %u rows are one run, no gaps, "
                    "%u pixels%s\n",
        ...
        for (uint32_t sb = 0; sb < nbx * nby; sb++) {
            ...
            for (uint32_t y = 0; y < 128u; y++) {
                for (uint32_t x = 0; x < 128u; x++) {
                    ...
                    cmp++;
                    eq += at32(s, (size_t)d * 65536u +
                                      (size_t)agc_tile_pixel(x, y) * 4u) == want;
                    drawn += want != FILL;
                }
            }
            if (cmp && eq == cmp) {
...
/* The report. Returns 0 with a verdict, 1 when an arm matched nothing, 2 when the rows
 * cannot decide (a dump incomplete, a control that drew nothing or everything). */
static int judge(job_t *j, FILE *out, uint32_t *chosen) {
    ...
    fprintf(out, "control %s: %zu of %zu bytes dumped\n", j->control.name, cc,
            j->control.size);
    if (cc != j->control.size)
        missing = 1;
    uint32_t drawn = 0;
    for (uint32_t y = 0; y < j->h; y++) {
        for (uint32_t x = 0; x < j->w; x++)
            drawn += px(&j->control, j->w, x, y, 0) != FILL;
    }
    ...
    for (int a = 0; a < j->n_arms; a++) {
        ...
        const int pass =
            complete && !missing && tiled == j->w * j->h && rows != j->w * j->h;
        fprintf(
            out,
    ...
    if (missing) {
        fprintf(
            out,
            "VERDICT: undecided - a dump is incomplete, or the control drew too little "
    ...
    fprintf(out,
            "VERDICT: the colour block draws 64KB_R_X as the display scans it; "
            "CB_COLOR0_ATTRIB3 0x%08x (%s)\n",
            j->arms[verdict].attrib3, j->arms[verdict].name);
    if (chosen)
        *chosen = j->arms[verdict].attrib3;
    return 0;
}
...
static int job_init(job_t *j, const char *check, const char *key, uint32_t w,
                    uint32_t h, const char *control) {
    memset(j, 0, sizeof(*j));
    j->check = check;
    j->key = key;
    j->w = w;
...
static int job_arm(job_t *j, const char *name, uint32_t attrib3) {
    if (j->n_arms >= MAX_ARMS)
        return -1;
    return surface_init(&j->arms[j->n_arms++], name, attrib3, 1,
                        tiled_size(j->w, j->h));
}
...
static int self_test_one_block(void) {
    ...
    for (int round = 0; round < 2; round++) {
        ...
        emit_rows(f, check, "control-linear", "color-target", lin, sizeof(lin),
                  SIZE_MAX);
        /* The right layout, and the wrong one: the same picture written as rows. Round
         * 1 drops the tiled arm's last rows, which has to read as undecided. */
        ...
        if (job_init(&j, check, "color-target", W, H, "control-linear") != 0 ||
            job_arm(&j, "arm1-rx", 0x08c6c000u) != 0 ||
            job_arm(&j, "arm2-rx-mesa", 0x0dc6c000u) != 0) {
            return 1;
        }
        read_rows(&j, f);
        fclose(f);
        ...
        if (round == 0 && !(rc == 0 && chosen == 0x08c6c000u))
            failures++;
        if (round == 1 && rc != 2)
            failures++;
        job_free(&j);
...
static int self_test_multiblock(void) {
    ...
    if (!f)
        return 1;
    emit_rows(f, check, "control-linear-256", "color-target", lin, sizeof(lin),
              SIZE_MAX);
    emit_rows(f, check, "arm1-rx", "color-target", rowmajor, sizeof(rowmajor),
              SIZE_MAX);
    emit_rows(f, check, "arm2-transposed", "color-target", transposed,
              sizeof(transposed), SIZE_MAX);
    rewind(f);
    job_t j;
    if (job_init(&j, check, "color-target", W, H, "control-linear-256") != 0 ||
        job_arm(&j, "arm1-rx", 0x08c6c000u) != 0 ||
        job_arm(&j, "arm2-transposed", 0x0dc6c000u) != 0) {
        fclose(f);
        return 1;
    }
    read_rows(&j, f);
...
/*
 * The shape mode: one tiled arm, no control. Reports whether the tool's block order is
 * the only one that detiles the dump into a triangle, and whether the pixels it finds
 * are the number the check itself reported. Returns 0 when both hold, 1 when they do
 * not.
 */
static int shape_report(job_t *j, const char *arm, FILE *out) {
    surface_t *s = NULL;
    for (int a = 0; a < j->n_arms; a++) {
        if (strcmp(j->arms[a].name, arm) == 0)
            s = &j->arms[a];
    }
    if (!s) {
    ...
    fprintf(out,
            "  the tool's order: %u of %u rows are one run, %u gap(s), %u pixels drawn",
            clean, touched, gaps, drawn);
    if (have_reported)
        fprintf(out, "; the check reported %u\n", reported);
    else
        fprintf(out, "; no modified-pixels row to compare\n");
    if (orders == 0) {
    ...
    if (clean != touched || gaps != 0u) {
        fprintf(out,
                "VERDICT: the tool's block order is NOT the one - %u of %u rows break, "
                "%u gap(s)\n",
    ...
    if (orders > 1) {
        fprintf(
            out,
            "VERDICT: undecided - %d block orders give one run a row, so this picture "
    ...
    if (have_reported && drawn != reported) {
        fprintf(
            out,
            "VERDICT: the shape is clean but holds %u pixels where the check counted "
            "%u\n",
            drawn, reported);
        return 1;
    }
    fprintf(out, "VERDICT: the blocks run in the tool's own order, and it is the only "
                 "order that "
                 "does\n");
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    if (!log) {
        fprintf(
            stderr,
            "usage: rx_check --self-test | --log OBS_LOG [--check NAME] [--key KEY]\n"
            "       [--control ARM] [--arm NAME=ATTRIB3 ... | --arm none] [--extent "
            "WxH]\n");
        return 2;
    }
    if (default_arms) {
    ...
    job_t j;
    if (job_init(&j, check, key, w, h, control) != 0)
        return 2;
    for (int a = 0; a < n_arms; a++) {
        if (job_arm(&j, arm_names[a], arm_attrib3[a]) != 0)
            return 2;
    }
    read_rows(&j, in);
    fclose(in);
