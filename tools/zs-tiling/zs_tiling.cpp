...
static const unsigned kGbAddrConfig =
    0x00000004u; /* oops-mesa drm_device.c: 16 pipes */
...
static uint64_t addr_of(ADDR_HANDLE lib, const Surf &s, unsigned w, unsigned h,
...
static Result measure(unsigned gb, const Surf &s) {
    ...
    /* Three blocks across, two down (the shape tiling-compare uses, and for its reason:
     * a 2 x 2 grid cannot tell row-major blocks from column-major). */
    ...
    while ((1u << xbits) < r.blk_w)
        xbits++;
    while ((1u << ybits) < r.blk_h)
        ybits++;
    for (unsigned b = 0; b < xbits; b++)
        r.xb.push_back((uint32_t)addr_of(lib, s, w, h, so.pitch, 1u << b, 0));
    for (unsigned b = 0; b < ybits; b++)
        r.yb.push_back((uint32_t)addr_of(lib, s, w, h, so.pitch, 0, 1u << b));
    ...
    for (unsigned y = 0; y < h; y++) {
        for (unsigned x = 0; x < w; x++) {
            ...
            uint64_t want =
                (uint64_t)((y / r.blk_h) * blocks_x + (x / r.blk_w)) * 65536u;
            uint32_t in_blk = 0;
            for (unsigned b = 0; b < xbits; b++)
                if ((lx >> b) & 1u)
                    in_blk ^= r.xb[b];
...
static void print_vec(const char *label, const std::vector<uint32_t> &v) {
    ...
    for (size_t i = 0; i < v.size(); i++)
        printf("%s0x%05x", i ? ", " : "", v[i]);
    printf("}\n");
}

int main(void) {
    const Surf surfs[2] = {{"depth, Z_32_FLOAT", 32, false},
                           {"stencil, STENCIL_8", 8, true}};
    printf(
        "zs-tiling: ADDR_SW_64KB_Z_X, family 0x%02X revision 0x%02X, pipeBankXor 0\n\n",
    ...
    for (const Surf &s : surfs) {
        ...
        printf("    block %u x %u; a 3 x 2 block surface is pitch %u, height %u, %llu "
               "bytes\n",
        ...
        printf("    control, 8 pipes: %s\n\n",
               !c.ok ? "addrlib refused it"
                     : (same ? "SAME basis - the control sees nothing"
